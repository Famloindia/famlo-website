...
export default async function BookingPage({
  ...
  const home: HomeCardRecord = {
    ...
    amenities: asStringArray(family?.amenities).length > 0 ? asStringArray(family?.amenities) : meta.amenities ?? asStringArray(host?.amenities),
    ...
    platformCommissionPct: asNumber(family?.platform_commission_pct ?? host?.platform_commission_pct) ?? DEFAULT_COMMISSION_PCT,
  ...
  const primaryStayUnit =
    ...
    stayUnits[0] ??
    null;
  const bookingHomeAmenities =
    primaryStayUnit?.amenities.length
      ? primaryStayUnit.amenities
      : asStringArray(family?.amenities).length > 0
        ? asStringArray(family?.amenities)
        : meta.amenities ?? asStringArray(host?.amenities);
