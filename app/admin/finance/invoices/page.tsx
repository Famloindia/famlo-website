...
export default async function AdminFinanceInvoicesPage() {
  ...
  return (
    <AdminLayout admin={{ id: "system-admin", name: "Famlo Admin", email: "admin@famlo.in" }} activeTab="finance" killSwitchActive={pageContext.killSwitchActive}>
      <AdminFinanceHeader title="Invoices and credit notes" description="Generate, download, and review guest invoices, platform-fee invoices, Famlo Pro GST receipts, and credit notes through guarded routes only." nav={getAdminFinanceNav("/admin/finance/invoices")} />
      ...
      {rows.length === 0 ? (
        ...
        <Card>
          <DataTable
            headers={["Document", "Reference", "Status", "Amount", "Email", "WhatsApp", "Downloads", "Actions"]}
            rows={rows.map((row) => [
              ...
              row.whatsappStatus ? <StatusPill key="whatsapp" value={row.whatsappStatus} /> : "Not queued",
              ...
              <div key="actions" style={{ display: "grid", gap: "8px", minWidth: "180px" }}>
                {row.kind === "host_pro_invoice" ? (
                  <div style={{ fontSize: "12px", color: "rgba(255,255,255,0.65)", lineHeight: 1.5 }}>
                    Generated from a paid Famlo Pro billing order. Retry email or WhatsApp is not exposed here yet.
                  </div>
                ) : (
                  <>
