...
export default async function HostDashboardPage({
  ...
  const { data: bookingRowsV2, error: bookingV2Error } =
    hostIds.length > 0
      ? await supabase
          .from("bookings_v2")
          .select([
  ...
  const bookingRows: Array<Record<string, unknown>> = ((bookingRowsV2 ?? []) as unknown as Array<Record<string, unknown>>).map((row) => {
