...
export default async function ProNewPropertyPage(): Promise<React.JSX.Element> {
  ...
  const canCreateProProperty = Boolean(famloProEnabled && access?.allowed && access.status === "active");
  const lockMessage =
    access?.status === "grace"
      ? "Famlo Pro property add-ons are blocked during grace. Renew your paid Pro plan first, then buy the Famlo Pro property add-on."
      : access?.status === "payment_failed" || access?.status === "halted"
        ? "Famlo Pro property add-ons are blocked until billing is active again. Fix or renew the Pro plan first."
        : "Only active paid Famlo Pro sessions can create another Famlo Pro property from this route.";

  if (!canCreateProProperty) {
    return (
      <main style={pageStyle}>
        <section style={cardStyle}>
          <div style={{ fontSize: 12, fontWeight: 900, letterSpacing: "0.08em", textTransform: "uppercase", color: "#93c5fd" }}>
