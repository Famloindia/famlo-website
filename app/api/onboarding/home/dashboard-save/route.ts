...
async function syncCanonicalHostReel(params: {
  supabase: ReturnType<typeof createAdminSupabaseClient>;
  familyId: string;
  publicUrl?: string;
  storageKey?: string;
  ...
  const { data: hostRow, error: hostLookupError } = await params.supabase
    .from("hosts")
    .select("id,user_id")
    .eq("legacy_family_id", params.familyId)
    .maybeSingle();

  if (hostLookupError) {
    return hostLookupError.message;
  }

  const hostId = hostRow && typeof hostRow.id === "string" ? hostRow.id : null;
  const hostUserId = hostRow && typeof hostRow.user_id === "string" ? hostRow.user_id : null;
  if (!hostId) {
    return "Host lookup missing for reel sync.";
  }

  let existingReelId: string | null = null;
  const existingPreferred = await params.supabase
  ...
  if (existingPreferred.error) {
    if (!isSchemaCompatibilityError(existingPreferred.error)) {
      return existingPreferred.error.message;
    }

    const existingCompat = await params.supabase
      .from("host_property_reels")
      .select("id")
      .eq("family_id", params.familyId)
      .limit(1)
    ...
    if (existingCompat.error) {
      return existingCompat.error.message;
    }

    existingReelId = existingCompat.data && typeof existingCompat.data.id === "string" ? existingCompat.data.id : null;
  } else {
    existingReelId = existingPreferred.data && typeof existingPreferred.data.id === "string" ? existingPreferred.data.id : null;
  }

  const updateCandidates = [
    {
      storage_key: params.storageKey ?? publicUrl,
      public_url: publicUrl,
      mime_type: params.mimeType ?? "video/mp4",
      size_bytes: params.sizeBytes ?? null,
      is_featured: true,
      status: "active",
      updated_at: params.uploadedAt ?? new Date().toISOString(),
    },
    {
      r2_key: params.storageKey ?? publicUrl,
      public_url: publicUrl,
      mime_type: params.mimeType ?? "video/mp4",
      size_bytes: params.sizeBytes ?? null,
      is_featured: true,
  ...
  if (existingReelId) {
    ...
    return "Unable to update canonical host reel metadata.";
  }
  ...
  for (const candidate of insertCandidates) {
    const { error } = await params.supabase.from("host_property_reels").insert(candidate as never);
    if (!error) {
      return null;
    }
    if (!isSchemaCompatibilityError(error)) {
  ...
  return "Unable to insert canonical host reel metadata.";
}
...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  const listingAmenitiesProvided = hasOwn(body.listing, "amenities");
  const nextAmenitiesList = listingAmenitiesProvided ? parseStringList(body.listing?.amenities) : undefined;
  ...
  try {
    ...
    const nextMeta = {
      ...
      hostReelStorageKey: toTrimmedString(body.listing?.hostReelStorageKey) ?? currentMeta.hostReelStorageKey,
      hostReelPublicUrl: toTrimmedString(body.listing?.hostReelPublicUrl) ?? currentMeta.hostReelPublicUrl,
      hostReelMimeType: toTrimmedString(body.listing?.hostReelMimeType) ?? currentMeta.hostReelMimeType,
      hostReelSizeBytes: parseNullableNumber(body.listing?.hostReelSizeBytes) ?? currentMeta.hostReelSizeBytes,
      hostReelUploadedAt: toTrimmedString(body.listing?.hostReelUploadedAt) ?? currentMeta.hostReelUploadedAt,
      ...
      gstin:
        normalizeGstin(body.compliancePatch?.gstin ?? body.compliancePatch?.gstNumber) || currentMeta.gstin,
      ...
      platformAgreementAcceptedAt:
        toTrimmedString(body.compliancePatch?.platformAgreementAcceptedAt) ?? currentMeta.platformAgreementAcceptedAt,
    };
    ...
    const familyPatch = cleanPatch({
      ...
      amenities:
        listingAmenitiesProvided ? nextAmenitiesList ?? [] : undefined,
      house_rules:
      ...
      gstin:
        normalizeGstin(body.compliancePatch?.gstin ?? body.compliancePatch?.gstNumber) || undefined,
    ...
    if (listingAmenitiesProvided) {
      const { data: stayUnits, error: stayUnitLookupError } = await supabase
        .from("stay_units_v2")
        .select("id,is_primary,sort_order")
        .eq("legacy_family_id", body.familyId)
        .order("is_primary", { ascending: false })
        .order("sort_order", { ascending: true })
        .order("updated_at", { ascending: false })
        .limit(5);

      if (stayUnitLookupError) {
        console.warn("[dashboard-save] stay-units:lookup:error", {
          familyId: body.familyId,
          code: "code" in stayUnitLookupError ? stayUnitLookupError.code : null,
          message: stayUnitLookupError.message,
          details: "details" in stayUnitLookupError ? stayUnitLookupError.details : null,
          hint: "hint" in stayUnitLookupError ? stayUnitLookupError.hint : null,
        });
        warnings.push(`Stay-unit amenities sync skipped: ${stayUnitLookupError.message}`);
      } else {
        const primaryUnits = ((stayUnits ?? []) as Array<Record<string, unknown>>)
          .filter((unit) => typeof unit.id === "string" && unit.id.length > 0)
          .filter((unit, index, items) => unit.is_primary === true || (index === 0 && !items.some((row) => row.is_primary === true)));

        for (const stayUnit of primaryUnits) {
          const { error: stayUnitUpdateError, strippedColumns } = await updateWithSchemaFallback(
            supabase,
            "stay_units_v2",
            {
              amenities: nextAmenitiesList ?? [],
          ...
          if (strippedColumns.length > 0) {
            console.warn("[dashboard-save] stay-units:amenities:schema-fallback", {
              familyId: body.familyId,
          ...
          if (stayUnitUpdateError) {
            console.warn("[dashboard-save] stay-units:amenities:update:error", {
              familyId: body.familyId,
              stayUnitId: stayUnit.id,
              errorMessage: stayUnitUpdateError instanceof Error ? stayUnitUpdateError.message : String(stayUnitUpdateError),
              error: stayUnitUpdateError,
            });
            warnings.push(`Stay-unit amenities sync skipped: ${stayUnitUpdateError instanceof Error ? stayUnitUpdateError.message : "unknown error"}`);
            break;
          }
        }
      }
    ...
    if (draftLookupError) {
      ...
      const nextDraftPayload = {
        ...
        hostReelStorageKey: toTrimmedString(body.listing?.hostReelStorageKey) ?? existingPayload.hostReelStorageKey,
        hostReelPublicUrl: toTrimmedString(body.listing?.hostReelPublicUrl) ?? existingPayload.hostReelPublicUrl,
        hostReelMimeType: toTrimmedString(body.listing?.hostReelMimeType) ?? existingPayload.hostReelMimeType,
        hostReelSizeBytes: parseNullableNumber(body.listing?.hostReelSizeBytes) ?? existingPayload.hostReelSizeBytes,
        hostReelUploadedAt: toTrimmedString(body.listing?.hostReelUploadedAt) ?? existingPayload.hostReelUploadedAt,
        ...
        gstin:
          normalizeGstin(body.compliancePatch?.gstin ?? body.compliancePatch?.gstNumber) ||
          (typeof existingPayload.gstin === "string" ? existingPayload.gstin : undefined),
        bookingRequiresHostApproval:
          typeof body.schedule?.bookingRequiresHostApproval === "boolean"
            ? body.schedule.bookingRequiresHostApproval
    ...
    const reelSyncError = await syncCanonicalHostReel({
      supabase,
      familyId: body.familyId,
      publicUrl: nextMeta.hostReelPublicUrl,
      storageKey: nextMeta.hostReelStorageKey,
      mimeType: nextMeta.hostReelMimeType,
      sizeBytes: nextMeta.hostReelSizeBytes ?? null,
      uploadedAt: nextMeta.hostReelUploadedAt,
    });
    if (reelSyncError) {
      console.warn("[dashboard-save] reel:sync:warning", {
        familyId: body.familyId,
        errorMessage: reelSyncError,
      });
      warnings.push(`Reel sync skipped: ${reelSyncError}`);
    }
