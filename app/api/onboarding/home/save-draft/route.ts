...
function collectRoomPhotoUrls(rooms: unknown[]): string[] {
  return rooms.flatMap((room) => {
    const record = asObject(room);
    return [
      ...asStringArray(record.roomPhotos),
      ...asStringArray(record.photos),
    ];
  });
}
...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  try {
    ...
    const normalizedGstin = normalizeGstin(
      compliancePatch.gstin ?? payloadPatch.gstin ?? payloadPatch.gstNumber ?? compliancePatch.gstNumber
    );
    const agreementState = derivePlatformAgreementState({
      platformAgreementAccepted: payloadPatch.platformAgreementAccepted ?? compliancePatch.platformAgreementAccepted,
      platformAgreementAcceptedAt: payloadPatch.platformAgreementAcceptedAt ?? compliancePatch.platformAgreementAcceptedAt,
      hostAgreementAccepted: payloadPatch.hostAgreementAccepted ?? compliancePatch.hostAgreementAccepted,
      hostAgreementAcceptedAt: payloadPatch.hostAgreementAcceptedAt ?? compliancePatch.hostAgreementAcceptedAt,
      termsPrivacyAccepted: payloadPatch.termsPrivacyAccepted ?? compliancePatch.termsPrivacyAccepted,
      commissionAgreementAccepted:
        payloadPatch.commissionAgreementAccepted ?? compliancePatch.commissionAgreementAccepted,
      codeOfConductAccepted: payloadPatch.codeOfConductAccepted ?? compliancePatch.codeOfConductAccepted,
      cancellationPolicyAccepted:
        payloadPatch.cancellationPolicyAccepted ?? compliancePatch.cancellationPolicyAccepted,
    });
    const agreementCompatibilityPatch = buildPlatformAgreementCompatibilityPatch(
      agreementState.accepted,
      agreementState.acceptedAt
    );
    ...
    const roomPhotoUrls = collectRoomPhotoUrls(rooms);
    const listingPhotos =
      Array.isArray(payloadPatch.photos) && payloadPatch.photos.length > 0
        ? payloadPatch.photos
        : roomPhotoUrls.length > 0
          ? roomPhotoUrls
          : Array.isArray(payloadPatch.hostGalleryPhotos)
    ...
    const patch: any = {
      ...
      hostReelStorageKey: asString(payloadPatch.hostReelStorageKey),
      hostReelPublicUrl: asString(payloadPatch.hostReelPublicUrl),
      hostReelMimeType: asString(payloadPatch.hostReelMimeType),
      hostReelSizeBytes: asNumberOrNull(payloadPatch.hostReelSizeBytes),
      hostReelUploadedAt: asString(payloadPatch.hostReelUploadedAt),
      gstin: normalizedGstin,
      platformAgreementAcceptedAt: agreementState.acceptedAt,
    ...
    patch.payloadPatch = {
      ...
      gstNumber: normalizedGstin || asString(payloadPatch.gstNumber) || asString(compliancePatch.gstNumber),
      gstin: normalizedGstin,
    ...
    patch.compliancePatch = {
      ...compliancePatch,
      gstin: normalizedGstin,
      gstNumber: normalizedGstin || asString(compliancePatch.gstNumber),
      hostReelStorageKey: asString(payloadPatch.hostReelStorageKey),
      hostReelPublicUrl: asString(payloadPatch.hostReelPublicUrl),
      hostReelMimeType: asString(payloadPatch.hostReelMimeType),
      hostReelSizeBytes: asNumberOrNull(payloadPatch.hostReelSizeBytes),
