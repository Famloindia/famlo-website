...
import { loadFreePmsCalendarSnapshot } from "@/lib/free-pms-calendar";
...
export async function POST(request: Request): Promise<NextResponse> {
  try {
    ...
    const snapshot = await loadFreePmsCalendarSnapshot(supabase, {
      familyId,
      dateFrom,
      dateTo,
    ...
    console.error("[host.free-pms.calendar.snapshot] error", error);
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Failed to load Free calendar snapshot." },
      { status: 500 }
    );
  }
}
