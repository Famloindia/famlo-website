...
export async function POST(request: Request): Promise<NextResponse> {
  try {
    ...
    if (access.status !== "active") {
      return NextResponse.json({ error: "Famlo Pro add-ons are only available during an active paid period." }, { status: 403 });
    }
    ...
    if (!addonOrderId) {
      ...
      return NextResponse.json(
        {
          error: "Property add-on payment is required before creation.",
