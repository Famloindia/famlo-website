...
export async function POST(request: Request): Promise<NextResponse> {
  try {
      ...
      return NextResponse.json(
        {
          ok: false,
          status: "unauthorized",
          message: "You can only run this Channex action for your own authenticated property workspace.",
        },
        { status: 401 }
      );
    }
    ...
    if (action !== "create_property" && !authorizedResource.isAdmin) {
      return NextResponse.json(
        {
          ok: false,
          status: "operator_required",
          message: "This Channex action still requires operator access.",
        },
        { status: 403 }
      );
    }
