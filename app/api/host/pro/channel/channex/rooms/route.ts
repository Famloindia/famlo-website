...
import { createChannexRoomType, fetchChannexPropertyById, getChannexConfigSummary } from "@/lib/channel-providers/channex/client";
...
export async function POST(request: Request): Promise<NextResponse> {
  try {
    ...
    const blockedMutation = await ensureChannexMutationAllowed({
      ...
      familyId,
      action: "create_room_type",
      route: "/api/host/pro/channel/channex/rooms",
    });
    ...
    if (!propertyLookup.ok || !propertyLookup.data?.id) {
      const message = "Saved Channex property mapping is stale or inaccessible. Re-run Create staging property to repair the mapping before creating rooms.";
