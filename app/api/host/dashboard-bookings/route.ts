...
async function loadFamilyRows(
  ...
  const initialResult = await supabase
    .from("families")
    .select("id,name,property_name,city,state,village,platform_commission_pct")
    .in("id", familyIds);

  if (initialResult.error && isMissingColumnError(initialResult.error, "property_name")) {
    const fallbackResult = await supabase
      .from("families")
      .select("id,name,city,state,village,platform_commission_pct")
      .in("id", familyIds);
  ...
  try {
    ...
    if (summaryOnly) {
      ...
      const totalEarnings = revenueRows.reduce((acc, row) => {
        ...
        const familyForBooking = familyIdByHostId.get(typeof row.host_id === "string" ? row.host_id : "") ?? familyId;
        const commissionPct = familyCommissionById.get(familyForBooking) ?? 16;
        return acc + Math.round(gross * ((100 - commissionPct) / 100));
      }, 0);
