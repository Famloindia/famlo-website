...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    return NextResponse.json({
      ...
      publicExportUrl: buildCalendarExportUrl({ hostId: ownerId, token: exportToken, baseUrl: requestBaseUrl }),
    });
  } catch (error) {
    return NextResponse.json(
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (body.action === "regenerate_export_url") {
      ...
      return NextResponse.json({
        success: true,
        publicExportUrl: buildCalendarExportUrl({ hostId: ownerId, token: exportToken, baseUrl: request.nextUrl.origin }),
      });
    }
