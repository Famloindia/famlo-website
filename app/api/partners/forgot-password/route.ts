...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  if (!partnerId || !email || !newPassword) {
    return NextResponse.json(
      { error: "Partner ID, approved email, and new password are required." },
      { status: 400 }
    );
  }
  ...
  if (familyRow?.user_id) {
    ...
    if (String((userRecord as { email?: string | null } | null)?.email ?? "").toLowerCase() === email) {
  ...
  return NextResponse.json(
    { error: "That partner ID and approved email do not match any Famlo Homes host." },
    { status: 404 }
  );
}
