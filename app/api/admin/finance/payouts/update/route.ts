...
export async function POST(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    if (reason.length < 10) {
      return NextResponse.json({ error: "An explicit admin reason of at least 10 characters is required." }, { status: 400 });
    }

    const supabase = createAdminSupabaseClient();
    const { data: payout, error: payoutError } = await supabase
      .from("payouts_v2")
      .select("id,booking_id,amount,status,partner_type,partner_user_id,net_transferable_amount,hold_reason,notes")
      .eq("id", payoutId)
      .maybeSingle();
    ...
    if (action === "mark_paid" && String(payout.status ?? "") === "paid") {
    ...
    await appendFinanceAuditLog(supabase, {
      ...
      actionType:
        action === "mark_paid"
          ? "payout_override_mark_paid"
          : action === "hold"
            ? "payout_override_hold"
            : "payout_override_release",
      resourceType: "payout",
      resourceId: payoutId,
      beforeValue: {
        status: typeof payout.status === "string" ? payout.status : null,
        holdReason: typeof payout.hold_reason === "string" ? payout.hold_reason : null,
      ...
      afterValue: {
        ...
        notes,
        transferReferenceRecorded: Boolean(transferReference),
      },
      reason,
    });
