...
export async function GET(): Promise<NextResponse> {
  try {
    ...
    const [paymentsRes, payoutsRes, refundsRes, reconRes, rulesRes, cancelledBookingsRes, proOrdersRes] = await Promise.all([
      supabase
        .from("payments_v2")
      ...
      supabase
        ...
        .select("subtotal_amount,gst_amount,status")
        .eq("status", "paid"),
    ]);
    ...
    const metrics: FinanceOverviewMetrics = {
      ...
      proRevenue: (proOrdersRes.data ?? []).reduce((sum, row) => sum + readAggregate(row.subtotal_amount), 0),
      proGst: (proOrdersRes.data ?? []).reduce((sum, row) => sum + readAggregate(row.gst_amount), 0),
