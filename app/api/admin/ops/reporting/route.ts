...
export async function GET(): Promise<NextResponse> {
  try {
    ...
    const channelOpsReadiness = await loadChannelOpsReadiness(supabase, { lookbackHours: 24 }).catch((error) => ({
      ...
      alerts: [
        {
          id: "channel_ops_load_failed",
          severity: "warning" as const,
          title: "Channel ops readiness could not be loaded",
          detail: error instanceof Error ? error.message : "Channel ops readiness query failed.",
          metric: "channel_ops_load_failed",
          value: 1,
          threshold: 0,
        },
      ],
      recentDeadLetters: [],
      runbook: ["Open the Channel Manager tab and inspect channel tables directly."],
    }));
