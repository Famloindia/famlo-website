...
import { applyFamilyWorkspacePolicyState, approveFamilyApplication } from "@/lib/family-approval";
...
type LegacyDecisionAction = "approve_marketplace" | "not_listed_allow_pro" | "block_completely";
...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  const body = (await request.json()) as {
    ...
    notes?: string;
    decisionAction?: LegacyDecisionAction;
  };
  ...
  if (
    ...
    (body.decisionAction === "not_listed_allow_pro" || body.decisionAction === "block_completely")
  ) {
    return NextResponse.json(
      {
        error:
          "This host decision must use /api/teams/vetting/decide so marketplace and Pro access stay in sync.",
      },
      { status: 400 }
    );
  }
  ...
  try {
    ...
    if (body.applicationType === "family" && body.status === "rejected") {
      await applyFamilyWorkspacePolicyState(supabase, {
        familyId:
          credentials?.profile_id ??
          (typeof currentApplication.approved_family_id === "string" ? currentApplication.approved_family_id : null) ??
          (typeof currentApplication.family_id === "string" ? currentApplication.family_id : null) ??
          (typeof currentApplication.legacy_family_id === "string" ? currentApplication.legacy_family_id : null),
        onboardingDraftId:
          typeof currentApplication.onboarding_draft_id === "string" ? currentApplication.onboarding_draft_id : null,
        applicationId: body.applicationId,
        propertyMarketplaceStatus: "rejected",
        trustStatus: "normal",
        reviewNotes: reviewNotes.length > 0 ? reviewNotes : "Rejected from admin console.",
