...
test("add-on checkout creates a room add-on order for an authorized host", async () => {
  ...
  const route = createHostProAddonCheckoutRouteHandlers({
    ...
    evaluateRuntimeSafety: (() => ({
      ok: true,
      scope: "pro_billing_payment_execution",
      appEnv: "staging",
      code: null,
      message: null,
    })) as never,
...
test("add-on checkout returns a clear disabled response without creating an order", async () => {
  ...
  const route = createHostProAddonCheckoutRouteHandlers({
    ...
    evaluateRuntimeSafety: (() => ({
      ...
      message: "Pro billing payment execution is disabled.",
    })) as never,
    resolveAuthorizedHostResource: (async () => ({
      familyId: "family-1",
      hostId: "host-1",
  ...
  const response = await route.POST(buildJsonRequest({ familyId: "family-1", addonType: "room" }) as never);
  const payload = (await response.json()) as { errorCode?: string; paymentExecutionEnabled?: boolean; message?: string };

  assert.equal(response.status, 503);
  assert.equal(checkoutCalled, false);
  assert.equal(payload.errorCode, "PRO_BILLING_PAYMENT_EXECUTION_DISABLED");
  assert.equal(payload.paymentExecutionEnabled, false);
  assert.equal(payload.message, "Payment execution disabled in local environment.");
});
