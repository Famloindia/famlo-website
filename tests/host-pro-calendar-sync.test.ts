...
test("room sync summary surfaces failed provisioning before showing synced cache state", () => {
  const summaries = summarizeCalendarRoomSyncStatuses({
    requestedRoomIds: ["room-1"],
    connected: true,
    lastSyncedAt: "2026-05-27T00:00:00.000Z",
    roomMappings: [{ stayUnitId: "room-1", externalRoomTypeId: null, syncStatus: "failed" }],
    ratePlans: [],
    jobs: [],
  });

  assert.equal(summaries[0]?.status, "failed");
  assert.equal(summaries[0]?.safeMessage, "Channex provisioning failed for this room. Retry room sync.");
});
...
test("calendar sync route supports background and status-only modes without exposing internals", () => {
  ...
  const snapshotSource = source("lib/host-pro-calendar-sync.ts");
  ...
  assert.match(snapshotSource, /requestedRoomIds\.length > 0 && unmappedRooms > 0/);
});
