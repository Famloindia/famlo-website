...
export function renderBookingReceipt(payload: JsonRecord): string {
  ...
  const paymentCollectMode = asString(payload.payment_collect_mode) ?? "UNKNOWN";
  const paymentCollectLabel =
  ...
  return shell("Famlo Booking Receipt", "Guest Document", [
    ...
    detailGrid([
      ...
      { label: "OTA Reservation ID", value: asString(payload.ota_reservation_id) ?? "N/A" },
      ...
      { label: "Guest Contact", value: asString(payload.guest_contact) ?? asString(payload.guest_email) ?? "Not provided" },
      { label: "Room", value: asString(payload.room_name) ?? "Room" },
      { label: "Check-in", value: asString(payload.check_in_date) ?? "N/A" },
      { label: "Check-out", value: asString(payload.check_out_date) ?? "N/A" },
      { label: "Nights", value: String(asNumber(payload.stay_nights)) },
      { label: "Guests", value: String(asNumber(payload.guests_count, 1)) },
      { label: "Channel", value: asString(payload.source_channel) ?? "Famlo" },
      { label: "Booking Status", value: asString(payload.booking_status) ?? "confirmed" },
      { label: "Booking Type", value: asString(payload.booking_type) ?? "Stay" },
      { label: "Booking Amount", value: `INR ${asNumber(payload.booking_amount).toLocaleString("en-IN")}` },
      { label: "Taxes / GST", value: `INR ${asNumber(payload.taxes_gst_amount).toLocaleString("en-IN")}` },
      { label: "Total Amount Paid", value: `INR ${totalPrice.toLocaleString("en-IN")}` },
      { label: "Payment Collect Mode", value: paymentCollectLabel },
    ...
    paymentCollectMode === "OTA_COLLECT"
      ? `<p style="margin-top:8px;font-size:12px;color:#64748b;font-weight:700">Payment collected by OTA. This receipt confirms booking details and stay charges.</p>`
      : "",
    `<p style="margin-top:8px;font-size:12px;color:#64748b;font-weight:700">This is a booking receipt generated by Famlo Pro.</p>`,
  ]);
}
...
export function renderHostBookingStatement(payload: JsonRecord): string {
  const commissionSource = asString(payload.ota_commission_source) ?? "not_available";
  const commissionSourceLabel =
    commissionSource === "actual"
      ? "Actual"
      : commissionSource === "estimated"
        ? `Estimated${asString(payload.ota_commission_rule_name) ? ` - ${asString(payload.ota_commission_rule_name)}` : ""}`
        : "Not available";
  const collectMode = asString(payload.payment_collect_mode) ?? "UNKNOWN";
  const collectModeLabel =
    collectMode === "OTA_COLLECT"
      ? "OTA collected"
      : collectMode === "PROPERTY_COLLECT"
        ? "Pay at property"
        : collectMode === "FAMLO_COLLECT"
          ? "Direct online"
          : collectMode;
  return shell("Famlo Host Booking Statement", "Host Document", [
    `<p style="font-size:15px;line-height:1.7;color:#334155">This statement summarizes the booking value, channel source, fees, refund adjustments, and host payout view for one reservation.</p>`,
    detailGrid([
      { label: "Statement No.", value: asString(payload.statement_number) ?? "N/A" },
      { label: "Booking ID", value: asString(payload.booking_id) ?? "N/A" },
      { label: "External Booking ID", value: asString(payload.external_booking_id) ?? "N/A" },
      { label: "Property", value: asString(payload.property_name) ?? "Famlo Stay" },
      { label: "Room", value: asString(payload.room_name) ?? "Room" },
      { label: "Guest", value: asString(payload.guest_name) ?? "Guest" },
      { label: "Source", value: asString(payload.source_channel) ?? "Famlo" },
      { label: "Check-in", value: asString(payload.check_in_date) ?? "N/A" },
      { label: "Check-out", value: asString(payload.check_out_date) ?? "N/A" },
      { label: "Booking Status", value: asString(payload.booking_status) ?? "unknown" },
      { label: "Payment Collect Mode", value: collectModeLabel },
      { label: "Payment Status", value: asString(payload.payment_status) ?? "unknown" },
      { label: "Settlement Status", value: asString(payload.settlement_status) ?? "pending_reconciliation" },
      { label: "Gross Booking Amount", value: `INR ${asNumber(payload.gross_booking_amount).toLocaleString("en-IN")}` },
      { label: "OTA Commission", value: `INR ${asNumber(payload.ota_commission_amount).toLocaleString("en-IN")}` },
      { label: "Commission Source", value: commissionSourceLabel },
      { label: "GST on OTA Commission", value: `INR ${asNumber(payload.ota_commission_gst_amount).toLocaleString("en-IN")}` },
      { label: "Famlo Fee", value: `INR ${asNumber(payload.platform_fee_amount).toLocaleString("en-IN")}` },
      { label: "GST / Tax", value: `INR ${asNumber(payload.tax_amount).toLocaleString("en-IN")}` },
      { label: "Refund / Credit Note", value: `INR ${asNumber(payload.refund_adjustment_amount).toLocaleString("en-IN")}` },
      { label: "Host Payout", value: `INR ${asNumber(payload.host_payout_amount).toLocaleString("en-IN")}` },
      { label: "Payout Status", value: asString(payload.payout_status) ?? "pending" },
    ]),
    `<div style="padding:18px;border-radius:18px;background:#eff6ff;border:1px solid #bfdbfe">
      <div style="font-size:12px;font-weight:800;text-transform:uppercase;letter-spacing:.08em;color:#1d4ed8">Data Source</div>
      <p style="margin:10px 0 0;font-size:14px;line-height:1.7;color:#1e3a8a">${escapeHtml(asString(payload.data_source_note) ?? "Generated from current Famlo booking, folio, payout, and channel records.")}</p>
    </div>`,
    `<p style="margin-top:20px;font-size:12px;color:#64748b;font-weight:700">Note: This is a system-generated host statement. Final settlement depends on verified payout and compliance records.</p>`,
  ]);
}
