...
function projectDay(input: {
  ...
  const shouldUseOtaAvailability =
    otaAvailability != null && !isEventNewer(bookingInventoryEvent, otaSyncEvent);
  const availableUnits = shouldUseOtaAvailability ? Math.max(0, otaAvailability) : locallyDerivedAvailableUnits;
  const stopSell = asBoolean(restrictionPayload.stop_sell, otaPayload.stopSell ?? input.rules.stopSellDefault);
