...
function eventCreatedAt(event: JsonRecord | null): string | null {
  return asString(event?.created_at);
}

function isEventNewer(left: JsonRecord | null, right: JsonRecord | null): boolean {
  const leftCreatedAt = eventCreatedAt(left);
  const rightCreatedAt = eventCreatedAt(right);
  if (!leftCreatedAt) return false;
  if (!rightCreatedAt) return true;
  return leftCreatedAt > rightCreatedAt;
}

function firstDefinedBoolean(...values: unknown[]): boolean | null {
...
function shouldUseOtaInventoryState(input: {
  otaPayload: ReturnType<typeof extractOtaSyncSnapshotPayload>;
  otaSyncEvent: JsonRecord | null;
  bookingInventoryEvent: JsonRecord | null;
}): boolean {
  const authoritative = firstDefinedBoolean(
    (input.otaSyncEvent?.payload as JsonRecord | null | undefined)?.availability_authoritative,
    (input.otaSyncEvent?.payload as JsonRecord | null | undefined)?.authoritative_availability,
    (input.otaSyncEvent?.payload as JsonRecord | null | undefined)?.use_ota_availability
  );
  return authoritative === true && input.otaPayload.availability != null && !isEventNewer(input.bookingInventoryEvent, input.otaSyncEvent);
}

export function extractOtaSyncSnapshotPayload(payload: JsonRecord | null | undefined): {
  ...
  const amount = firstDefinedNumber(source.amount, source.rate);
  const availability = firstDefinedNumber(source.availability);
  return {
    availability: availability != null ? Math.max(0, Math.trunc(availability)) : null,
    amount: amount != null ? normalizeInventoryRateAmount(amount) : null,
    stopSell: firstDefinedBoolean(source.stop_sell, source.stopSell, source.is_blocked),
    cta: firstDefinedBoolean(source.cta, source.closed_to_arrival),
    ctd: firstDefinedBoolean(source.ctd, source.closed_to_departure),
    minStayThrough: firstDefinedNumber(source.min_stay_through, source.minStayThrough, source.min_stay),
    minStayArrival: firstDefinedNumber(source.min_stay_arrival, source.minStayArrival),
    maxStay: firstDefinedNumber(source.max_stay, source.maxStay),
  };
}
...
function projectDay(input: {
  ...
  const otaSyncEvent = latestEventForDate(input.events, input.date, ["ota_sync_applied"]);
  const bookingInventoryEvent = latestEventForDate(input.events, input.date, [
    "booking_hold_created",
    "booking_hold_released",
    "booking_confirmed",
    "booking_cancelled",
  ...
  const otaPayload = extractOtaSyncSnapshotPayload((otaSyncEvent?.payload as JsonRecord | null) ?? {});
  const manualRateAmount =
    asString(rateEvent?.event_type) === "manual_rate_set" ? normalizeInventoryRateAmount(ratePayload.amount) : 0;
  const otaRateAmount = otaPayload.amount ?? 0;
  const effectiveRate =
    manualRateAmount > 0
      ? manualRateAmount
      : otaRateAmount > 0
  ...
  const cta = asBoolean(
    restrictionPayload.cta ?? restrictionPayload.closed_to_arrival,
    otaPayload.cta ?? input.rules.ctaDefault
  );
  const ctd = asBoolean(
    restrictionPayload.ctd ?? restrictionPayload.closed_to_departure,
    otaPayload.ctd ?? input.rules.ctdDefault
  );
  const minStay = Math.max(
    1,
    asNumber(restrictionPayload.min_stay ?? restrictionPayload.min_stay_through, otaPayload.minStayThrough ?? input.rules.minStayDays)
  );
  const minStayArrival = Math.max(
    1,
    asNumber(restrictionPayload.min_stay_arrival, otaPayload.minStayArrival ?? input.rules.minStayArrivalDays)
  );
  const maxStay = Math.max(minStay, asNumber(restrictionPayload.max_stay, otaPayload.maxStay ?? input.rules.maxStayDays));
  const locallyDerivedAvailableUnits = Math.max(0, allotmentLimit - confirmedUnits - holdUnits);
  const otaAvailability = otaPayload.availability;
  const shouldUseOtaAvailability = shouldUseOtaInventoryState({
    otaPayload,
    otaSyncEvent,
    bookingInventoryEvent,
  });
  const availableUnits = shouldUseOtaAvailability ? Math.max(0, otaAvailability ?? 0) : locallyDerivedAvailableUnits;
  const stopSell = asBoolean(
    restrictionPayload.stop_sell,
    shouldUseOtaAvailability ? otaPayload.stopSell ?? input.rules.stopSellDefault : input.rules.stopSellDefault
  );
  ...
  return {
    ...
    metadata: {
      ...
      ota_snapshot_availability: otaAvailability,
      ota_snapshot_rate: otaPayload.amount,
      ota_snapshot_event_id: asString(otaSyncEvent?.id),
      ota_snapshot_applied: shouldUseOtaAvailability || otaPayload.amount != null,
    },
  };
}
