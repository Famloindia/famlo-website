...
export function buildCalendarExportUrl(input: { hostId: string; token: string; baseUrl?: string | null }): string {
  const fallbackBase = getPublicSiteUrl();
  const providedBase = typeof input.baseUrl === "string" && input.baseUrl.trim().length > 0 ? input.baseUrl.trim() : fallbackBase;
  return new URL(`/ical/host/${input.hostId}/${input.token}.ics`, providedBase).toString();
}
