...
import {
  ...
  type FamloProEntitlementReason,
...
import { normalizeTrustStatus } from "@/lib/host-access-policy";
...
function toAccessResult(result: ProAccessStatusResult): HostProAccessResult {
  return {
    ...
    proActionsAllowed: result.proActionsAllowed,
    reason: result.reason,
  };
}

async function loadFamilyTrustStatusMap(
  ...
  const trustStatusMap: Record<string, string> = {};
  for (const row of (Array.isArray(data) ? (data as JsonRecord[]) : [])) {
    const familyId = asString(row.id);
    if (!familyId) continue;
    trustStatusMap[familyId] = normalizeTrustStatus(row);
  }
  return trustStatusMap;
}
...
export async function loadHostProAccessMap(
  ...
  const trustStatusMap = await loadFamilyTrustStatusMap(supabase, normalizedFamilyIds);
  ...
  for (const familyId of normalizedFamilyIds) {
    ...
    const result = toAccessResult(deriveProAccessStatus(familyRows[0] ?? null, { now }));
    resultMap[familyId] =
      trustStatusMap[familyId] === "blocked"
        ? {
            ...result,
            allowed: false,
            paidActive: false,
