...
export async function createPropertyMediaUploadTarget(input: {
  familyId: string;
  fileName: string;
  mimeType: string;
  sizeBytes: number;
  ...
  const validationError = validatePropertyMediaDirectUpload(input);
  if (validationError) {
    throw new Error(validationError);
  }

  const keyPrefix = `property-media/${input.familyId}/gallery/`;
  assertUploadConfig({
