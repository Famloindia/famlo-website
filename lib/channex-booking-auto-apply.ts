...
import { assertCanonicalInventoryAvailability, ensureProjectedInventory } from "@/lib/inventory";
import { getStayNightDateRange } from "@/lib/platform-utils";
...
type RoomMappingLookupResult = {
  stayUnitId: string | null;
  providerCode: string | null;
};

type IncomingGuestContact = {
  guestName: string;
...
function resolveIncomingGuestContact(rawPayload: JsonRecord, fallbackName: string | null): IncomingGuestContact {
  const attributes = asObject(rawPayload.attributes);
  const customer = asObject(attributes.customer);
  const firstRoom = asObject((Array.isArray(attributes.rooms) ? attributes.rooms : [])[0]);
  const roomOccupancy = asObject(firstRoom.occupancy);
  const bookingOccupancy = asObject(attributes.occupancy);

  const adultsCount =
    asNumberOrNull(roomOccupancy.adults) ??
    asNumberOrNull(bookingOccupancy.adults);
  const childrenCount =
    asNumberOrNull(roomOccupancy.children) ??
    asNumberOrNull(bookingOccupancy.children) ??
    0;
  const guestCount = Math.max(
    1,
    (adultsCount ?? 0) + Math.max(0, childrenCount ?? 0)
  );

  return {
    guestName: resolveIncomingGuestName(rawPayload, fallbackName),
    guestEmail:
      asStringOrNull(rawPayload.email) ??
      asStringOrNull(rawPayload.guest_email) ??
      asStringOrNull(rawPayload.customer_email) ??
      asStringOrNull(customer.mail),
    guestPhone:
      asStringOrNull(rawPayload.phone) ??
      asStringOrNull(rawPayload.guest_phone) ??
      asStringOrNull(rawPayload.customer_phone) ??
      asStringOrNull(customer.phone),
    guestCount,
...
function buildConflictSummary(input: {
  manualBlock: boolean;
  inventoryBlocked: boolean;
  capacityExceeded: boolean;
}): RevisionConflictSummary | null {
  const reasonCodes: string[] = [];
  const reasonLabels: string[] = [];

  if (input.manualBlock) {
    reasonCodes.push("channex_booking_conflicts_with_manual_block");
    reasonLabels.push("Conflicts with manual block");
  }
  if (input.inventoryBlocked) {
    reasonCodes.push("channex_booking_overbooks_inventory");
    reasonLabels.push("Overbooks room inventory");
  }
  if (input.capacityExceeded) {
    reasonCodes.push("channex_booking_exceeds_room_capacity");
    reasonLabels.push("Exceeds room capacity");
  }

  if (reasonCodes.length === 0) return null;

  return {
    message: reasonLabels.join(". "),
    reasonCodes,
    reasonLabels,
  };
}

async function markRevisionPendingReview(
  supabase: SupabaseClient,
  revision: RevisionRecord,
  input: {
    message: string;
  ...
  await supabase
    ...
    .update({
      ...
      raw_payload: {
        ...revision.raw_payload,
        review_title: "Channex booking needs review",
        review_conflict_reasons: input.reasonCodes,
        review_conflict_labels: input.reasonLabels,
        detected_guest_count: input.guestCount,
        max_guests: input.maxGuests,
        last_auto_apply_error: input.message,
        last_auto_apply_error_at: now,
        last_auto_apply_kind: "import",
      },
      updated_at: now,
    } as never)
    .eq("id", revision.id);
...
async function refreshInventoryAndQueueAriSync(
  supabase: SupabaseClient,
  input: {
    familyId: string;
    stayUnitIds: string[];
  ...
  for (const stayUnitId of input.stayUnitIds) {
    if (!stayUnitId) continue;
    await ensureProjectedInventory(supabase, {
  ...
  await enqueueBookingInventoryAriSyncJobs(supabase, {
    ...
    stayUnitIds: input.stayUnitIds.filter(Boolean),
    certificationScenario: input.certificationScenario,
    sourceUiAction: input.sourceUiAction,
    sourceRoute: "lib/channex-booking-auto-apply",
...
async function resolveMappedStayUnitForRevision(
  supabase: SupabaseClient,
  revision: RevisionRecord,
  preferredProviderCode: string
): Promise<RoomMappingLookupResult> {
  const providerCodes = Array.from(
    new Set(
      [preferredProviderCode, "channex"]
  ...
  for (const providerCode of providerCodes) {
    const { data: roomMappingRow, error: roomMappingError } = await supabase
      ...
      .select("stay_unit_id,provider_code")
    ...
    if (stayUnitId) {
      return {
        stayUnitId,
        providerCode: asStringOrNull(roomMappingRow?.provider_code) ?? providerCode,
      };
    }
  }
...
async function autoImportNewRevision(
  supabase: SupabaseClient,
  revision: RevisionRecord
): Promise<{ ok: true; bookingId: string } | { ok: false; message: string; reviewRequired?: boolean }> {
  ...
  const roomMapping = await resolveMappedStayUnitForRevision(supabase, revision, storageProviderCode);
  const stayUnitId = roomMapping.stayUnitId;
  if (!stayUnitId) {
  ...
  const guestContact = resolveIncomingGuestContact(revision.raw_payload, revision.guest_name);
  const maxGuests = asNumberOrNull(stayUnitRow?.max_guests);
  const capacityExceeded = maxGuests != null && guestContact.guestCount > maxGuests;
  let manualBlockConflict = false;
  let inventoryConflict = false;
  try {
    await assertCanonicalInventoryAvailability(supabase, {
      familyId: revision.family_id,
      stayUnitId,
      startDate: revision.arrival_date,
      endDate: revision.departure_date,
    });
  } catch (error) {
    const message = error instanceof Error ? error.message : "This room is not available for the selected date.";
    if (message.includes("manually blocked")) {
      manualBlockConflict = true;
    } else {
      inventoryConflict = true;
    }
  }
  const conflictSummary = buildConflictSummary({
    manualBlock: manualBlockConflict,
    inventoryBlocked: inventoryConflict,
    capacityExceeded,
  });
  if (conflictSummary) {
    await markRevisionPendingReview(supabase, revision, {
      message: conflictSummary.message,
      reasonCodes: conflictSummary.reasonCodes,
      reasonLabels: conflictSummary.reasonLabels,
      guestCount: guestContact.guestCount,
      maxGuests,
    });
    return {
      ok: false,
  ...
  const guestEmail = guestContact.guestEmail;
  const guestPhone = guestContact.guestPhone;
  const guestName = guestContact.guestName;
  ...
  const pricingSnapshot = {
    ...
    channel_guest_count: guestContact.guestCount,
    channel_guest_adults: guestContact.adultsCount,
    channel_guest_children: guestContact.childrenCount,
  ...
  const bookingPayload: Record<string, unknown> = {
    ...
    quarter_type: null,
    quarter_time: null,
    guests_count: guestContact.guestCount,
  ...
  await refreshInventoryAndQueueAriSync(supabase, {
    familyId: revision.family_id,
    stayUnitIds: [stayUnitId],
    startDate: revision.arrival_date,
    endDate: revision.departure_date,
    actorUserId: technicalUserId,
    sourceUiAction: "Channex inbound booking import",
    certificationScenario: "channex_inbound_booking_import",
  });
...
async function autoApplyCancellationRevision(
  ...
  if (currentStayUnitId) {
    await refreshInventoryAndQueueAriSync(supabase, {
      familyId: revision.family_id,
      stayUnitIds: [currentStayUnitId],
      startDate: asStringOrNull(linkedBooking.start_date) ?? revision.arrival_date ?? "",
      endDate: asStringOrNull(linkedBooking.end_date) ?? revision.departure_date ?? "",
      actorUserId: linkedHostId,
      sourceUiAction: "Channex inbound booking cancellation",
      certificationScenario: "channex_inbound_booking_cancellation",
    });
  }
...
async function autoApplyModificationRevision(
  ...
  const roomMapping = await resolveMappedStayUnitForRevision(supabase, revision, storageProviderCode);
  const targetStayUnitId = roomMapping.stayUnitId;
  if (!targetStayUnitId) {
    return { ok: false, message: "Mapped stay unit was not found for this external room type." };
  }
  ...
  const { data: stayUnitRow, error: stayUnitError } = await supabase
    .from("stay_units_v2")
    .select("id,host_id,legacy_family_id,max_guests")
  ...
  const guestContact = resolveIncomingGuestContact(
    revision.raw_payload,
  ...
  try {
    await assertCanonicalInventoryAvailability(supabase, {
      familyId: revision.family_id,
      stayUnitId: targetStayUnitId,
      startDate: assuredArrivalDate,
      endDate: assuredDepartureDate,
      excludeBookingId: revision.linked_booking_id,
    });
  } catch (error) {
    const message = error instanceof Error ? error.message : "This room is not available for the selected date.";
    if (message.includes("manually blocked")) {
  ...
  if (conflictSummary) {
    ...
    return { ok: false, message: conflictSummary.message };
  }
  const guestEmail = guestContact.guestEmail;
  const guestPhone = guestContact.guestPhone;
  const guestName = guestContact.guestName;
  ...
  const reprojectStartDate =
    currentStartDate < assuredArrivalDate ? currentStartDate : assuredArrivalDate;
  const reprojectEndDate =
    currentEndDate > assuredDepartureDate ? currentEndDate : assuredDepartureDate;
  await refreshInventoryAndQueueAriSync(supabase, {
    familyId: revision.family_id,
    stayUnitIds: Array.from(new Set([currentStayUnitId, targetStayUnitId].filter(Boolean) as string[])),
    startDate: reprojectStartDate,
    endDate: reprojectEndDate,
    actorUserId: linkedHostId,
    sourceUiAction: "Channex inbound booking modification",
    certificationScenario: "channex_inbound_booking_modification",
  });
  ...
  for (const rawRow of (rows ?? []) as Array<Record<string, unknown>>) {
    ...
    if (status === "new") {
      ...
      try {
        ...
        if (!importResult.ok) {
          if (importResult.reviewRequired) {
            pendingManualReviewCount += 1;
