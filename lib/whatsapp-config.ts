...
export function getWhatsAppRuntimeConfig(): WhatsAppRuntimeConfig {
  ...
  return {
    ...
    templateLanguage: value("WHATSAPP_TEMPLATE_LANGUAGE") ?? DEFAULT_TEMPLATE_LANGUAGE,
    templates: {
      bookingApproval: value("WHATSAPP_BOOKING_APPROVAL_TEMPLATE"),
      test: value("WHATSAPP_TEST_TEMPLATE"),
      guestMessage: value("WHATSAPP_GUEST_MESSAGE_TEMPLATE"),
    },
    siteUrl: value("NEXT_PUBLIC_SITE_URL"),
    cronSecret: value("CRON_SECRET"),
    stagingTesterPhone: normalizeMetaPhone(value("WHATSAPP_STAGING_TESTER_PHONE")),
    stagingRealDeliveryAllowed: enabled("FAMLO_ALLOW_STAGING_WHATSAPP_DELIVERY"),
  };
}
...
export function validateWhatsAppEnvironment(): {
  ...
  const required: Array<[string, string | null]> = [
    ...
    ["WHATSAPP_BOOKING_APPROVAL_TEMPLATE", config.templates.bookingApproval],
    ["WHATSAPP_TEST_TEMPLATE", config.templates.test],
    ["WHATSAPP_GUEST_MESSAGE_TEMPLATE", config.templates.guestMessage],
  ...
  if (!stagingTesterRestricted) {
    if (!config.stagingTesterPhone) missing.push("WHATSAPP_STAGING_TESTER_PHONE");
    if (!config.stagingRealDeliveryAllowed) missing.push("FAMLO_ALLOW_STAGING_WHATSAPP_DELIVERY");
  }
