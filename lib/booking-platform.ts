...
import { renderBookingReceipt, renderCompliancePack, renderHostBookingStatement, renderPayoutStatement } from "@/lib/document-templates";
import { estimateChannelCommission, loadChannelFinanceSettings } from "@/lib/channel-finance-settings";
...
export async function buildBookingReceiptDocument(
  ...
  const { data: booking, error: bookingError } = await supabase
    .from("bookings_v2")
    .select("id,booking_type,start_date,end_date,quarter_type,guests_count,total_price,status,payment_status,pricing_snapshot,user_id,host_id,hommie_id,users!user_id(name,email),hosts!host_id(id,display_name,city,state,legacy_family_id),hommie_profiles_v2!hommie_id(display_name,city,state)")
    .eq("id", bookingId)
    .maybeSingle();
  if (bookingError) throw bookingError;
  if (!booking) throw new Error("Booking not found.");
  ...
  const propertyLocation = [asString(family?.city) ?? asString(partner?.city), asString(family?.state) ?? asString(partner?.state)]
    .filter(Boolean)
    .join(", ");
  const bookingRecord = booking as JsonRecord;
  const pricingSnapshot = (bookingRecord.pricing_snapshot as JsonRecord | null) ?? {};
  const checkInDate = asString(bookingRecord.start_date);
  const checkOutDate = asString(bookingRecord.end_date);
  const stayNights =
    checkInDate && checkOutDate
      ? Math.max(
          0,
          Math.round(
            (new Date(`${checkOutDate}T00:00:00Z`).getTime() - new Date(`${checkInDate}T00:00:00Z`).getTime()) /
              86_400_000
          )
        )
      : 0;
  const paymentCollectMode =
    ...
    asString(pricingSnapshot.collect_mode) ??
    (asString(pricingSnapshot.channel_provider) || asString(pricingSnapshot.ota_name) ? "OTA_COLLECT" : "FAMLO_COLLECT");
  const sourceChannel =
    asString(pricingSnapshot.ota_name) ??
    asString(pricingSnapshot.channel_provider) ??
    asString(pricingSnapshot.source_channel) ??
    (asString(bookingRecord.booking_type) === "host_manual" ? "Famlo PMS" : "Famlo Direct");

  const payload: JsonRecord = {
    booking_id: asString(bookingRecord.id),
    receipt_number: `FR-${asString(bookingRecord.id)?.slice(0, 8).toUpperCase()}`,
    issued_at: new Date().toISOString(),
    booking_type: asString(bookingRecord.booking_type),
    ota_reservation_id:
      asString(pricingSnapshot.channel_external_booking_id) ??
      asString(pricingSnapshot.external_booking_id) ??
      asString(pricingSnapshot.ota_reservation_id),
    guest_name: asString(user?.name),
    guest_email: asString(user?.email),
    guest_contact: asString(pricingSnapshot.guest_phone) ?? asString(pricingSnapshot.guest_contact),
    partner_name: asString(partner?.display_name),
    host_name: asString(partner?.display_name),
    property_name: propertyName,
    property_location: propertyLocation,
    room_name:
      asString(pricingSnapshot.room_name) ??
      asString(pricingSnapshot.stay_unit_name) ??
      asString(pricingSnapshot.channel_room_name),
    stay_window: `${checkInDate} to ${checkOutDate}`,
    check_in_date: checkInDate,
    check_out_date: checkOutDate,
    stay_nights: stayNights,
    guests_count: asNumber(bookingRecord.guests_count, 1),
    source_channel: sourceChannel,
    booking_status: asString(bookingRecord.status) ?? "confirmed",
    slot_key: asString(bookingRecord.quarter_type),
    booking_amount: asNumber(bookingRecord.total_price),
    taxes_gst_amount:
      asNumber(pricingSnapshot.tax_amount) ||
      asNumber(pricingSnapshot.gst_amount) ||
...
export async function buildHostBookingStatementDocument(
  supabase: SupabaseClient,
  bookingId: string
): Promise<{ title: string; html: string; payload: JsonRecord }> {
  const { data: booking, error: bookingError } = await supabase
    .from("bookings_v2")
    .select("id,booking_type,start_date,end_date,guests_count,total_price,status,payment_status,pricing_snapshot,user_id,host_id,hosts!host_id(id,display_name,city,state,legacy_family_id),users!user_id(name,email)")
    .eq("id", bookingId)
    .maybeSingle();
  if (bookingError) throw bookingError;
  if (!booking) throw new Error("Booking not found.");
  ...
  const pricingSnapshot = (bookingRecord.pricing_snapshot as JsonRecord | null) ?? {};
  const hostLegacyFamilyId = asString((host as JsonRecord | null)?.legacy_family_id);
  const familyResult = hostLegacyFamilyId
    ? await supabase.from("families").select("id,name,property_name,city,state").eq("id", hostLegacyFamilyId).maybeSingle()
  ...
  const [
    ...
    supabase
      .from("payouts_v2")
      .select("id,amount,status,gross_booking_value,platform_fee,platform_fee_tax,gateway_fee_burden_amount,withholding_amount,hold_reason,created_at,paid_at")
      .eq("booking_id", bookingId)
      .order("created_at", { ascending: false })
      .limit(1),
    supabase
  ...
  if (payoutRowsResult.error) throw payoutRowsResult.error;
  if (folioRowsResult.error) throw folioRowsResult.error;
  if (settlementLineRowsResult.error) throw settlementLineRowsResult.error;
  if (platformInvoicesResult.error) throw platformInvoicesResult.error;
  if (creditNotesResult.error) throw creditNotesResult.error;

  const family = familyResult.data as JsonRecord | null;
  const latestPayout = ((payoutRowsResult.data ?? []) as JsonRecord[])[0] ?? null;
  const folio = ((folioRowsResult.data ?? []) as JsonRecord[])[0] ?? null;
  const settlementLine = ((settlementLineRowsResult.data ?? []) as JsonRecord[])[0] ?? null;
  const platformInvoice = ((platformInvoicesResult.data ?? []) as JsonRecord[])[0] ?? null;
  const creditNoteAmount = ((creditNotesResult.data ?? []) as JsonRecord[]).reduce(
    (sum, row) => sum + asNumber(row.total_reversal_amount),
    0
  );
  const financeSnapshot = (pricingSnapshot.finance_snapshot as JsonRecord | null) ?? {};
  const financeContract = (financeSnapshot.contract_v1 as JsonRecord | null) ?? {};
  const payoutBreakdown = (financeSnapshot.payout_breakdown as JsonRecord | null) ?? {};
  const grossBookingAmount =
    asNumber(latestPayout?.gross_booking_value) ||
    asNumber(folio?.guest_total_amount) ||
    asNumber(bookingRecord.total_price);
  const platformFeeAmount =
    asNumber(platformInvoice?.total_amount) ||
    asNumber(latestPayout?.platform_fee) ||
  ...
  const taxAmount =
    asNumber(latestPayout?.platform_fee_tax) ||
  ...
  const actualOtaCommissionAmount =
    asNumber(pricingSnapshot.ota_commission_amount) ||
    asNumber(pricingSnapshot.ota_commission) ||
    null;
  ...
  const sourceChannel =
    ...
    asString(pricingSnapshot.source_channel) ??
    (asString(bookingRecord.booking_type) === "host_manual" ? "Famlo PMS" : "Famlo Direct");
  let commissionEstimate = estimateChannelCommission({
    grossAmount: grossBookingAmount,
    actualCommissionAmount: actualOtaCommissionAmount,
    rules: [],
    sourceChannel,
  });
  if (commissionEstimate.source === "not_available" && hostLegacyFamilyId) {
    try {
      const financeSettings = await loadChannelFinanceSettings(supabase, hostLegacyFamilyId);
      commissionEstimate = estimateChannelCommission({
        grossAmount: grossBookingAmount,
        actualCommissionAmount: actualOtaCommissionAmount,
        rules: financeSettings.commissionRules,
        sourceChannel,
      });
    } catch (error) {
      console.warn("[booking-platform] unable to load Channel Finance rules for host statement:", error);
    }
  }
  const otaCommissionAmount = commissionEstimate.amount;
  const hostPayoutAmount =
    asNumber(latestPayout?.amount) ||
    asNumber(settlementLine?.amount) ||
    asNumber(folio?.host_payout_amount) ||
    asNumber(payoutBreakdown.host_net_payout) ||
    Math.max(
      0,
      grossBookingAmount -
        platformFeeAmount -
        ...
        (commissionEstimate.totalCommissionAmount ?? 0)
    );

  const payload: JsonRecord = {
    statement_number: `HS-${bookingId.slice(0, 8).toUpperCase()}`,
    booking_id: bookingId,
    external_booking_id:
      asString(pricingSnapshot.channel_external_booking_id) ??
      asString(pricingSnapshot.external_booking_id),
    ...
    guest_name:
      ...
      asString((user as JsonRecord | null)?.name) ??
      "Guest",
    guest_email:
      asString(pricingSnapshot.guest_email) ??
      asString((user as JsonRecord | null)?.email),
    source_channel: sourceChannel,
    check_in_date: asString(bookingRecord.start_date),
    check_out_date: asString(bookingRecord.end_date),
    guests_count: asNumber(bookingRecord.guests_count, 1),
    booking_status: asString(bookingRecord.status),
    payment_collect_mode:
      asString(pricingSnapshot.payment_collect_mode) ??
      asString(pricingSnapshot.collect_mode) ??
      (sourceChannel === "Famlo Direct" || sourceChannel === "Famlo PMS" ? "FAMLO_COLLECT" : "OTA_COLLECT"),
    payment_status: asString(folio?.payment_status) ?? asString(bookingRecord.payment_status),
    gross_booking_amount: grossBookingAmount,
    ota_commission_amount: otaCommissionAmount,
    ota_commission_source: commissionEstimate.source,
    ota_commission_rule_name: commissionEstimate.ruleName,
    ota_commission_gst_amount: commissionEstimate.gstAmount,
    platform_fee_amount: platformFeeAmount,
    tax_amount: taxAmount,
    refund_adjustment_amount: refundAdjustmentAmount,
    host_payout_amount: hostPayoutAmount,
    settlement_status: asString(settlementLine?.settlement_id) ? "matched" : "pending_reconciliation",
    payout_status: asString(latestPayout?.status) ?? asString(folio?.payment_status) ?? "pending",
    payout_id: asString(latestPayout?.id),
    data_source_note: latestPayout
      ? "Generated from booking, folio, invoice, credit-note, and payout records."
      : "Generated from booking, folio, invoice, and credit-note records. No final payout row exists yet, so payout values may be estimated from current booking finance data.",
  };

  return {
    title: "Famlo Host Booking Statement",
    html: renderHostBookingStatement(payload),
    payload,
  };
}
