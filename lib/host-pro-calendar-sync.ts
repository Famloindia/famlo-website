...
export function summarizeCalendarRoomSyncStatuses(input: {
  ...
  return input.requestedRoomIds.map((roomId) => {
    ...
    const roomProvisioningStatus = asString(roomMapping?.syncStatus);
    const rateProvisioningStatus = asString(ratePlan?.syncStatus);
    const provisioningFailed = roomProvisioningStatus === "failed" || rateProvisioningStatus === "failed";
    const provisioningPending =
      roomProvisioningStatus === "pending" ||
      roomProvisioningStatus === "retrying" ||
      rateProvisioningStatus === "pending" ||
      rateProvisioningStatus === "retrying";
    ...
    if (!input.connected) {
      ...
      status = "failed";
      safeMessage = "Channex provisioning failed for this room. Retry room sync.";
    } else if (provisioningPending) {
      status = "pending";
      safeMessage = "Channex provisioning is pending for this room.";
    } else if (!mapped) {
      status = "not_mapped";
...
export async function loadHostProCalendarSyncSnapshot(input: {
  ...
  if (
    isSchemaCompatibilityError(propertyResult.error) ||
    isSchemaCompatibilityError(roomMappingsResult.error) ||
    isSchemaCompatibilityError(ratePlansResult.error) ||
    isSchemaCompatibilityError(jobsResult.error) ||
    isSchemaCompatibilityError(logsResult.error)
  ) {
    return buildCalendarSyncMetadata({
      connected: false,
      observedAt,
      syncSource: "none",
      statusTitle: "Sync status unavailable",
      statusDetail: "Calendar sync status could not be read in this environment.",
    });
  }
  ...
  const failedRooms = roomStatuses.filter((entry) => entry.status === "failed").length;
  ...
  if (requestedRoomIds.length > 0 && unmappedRooms > 0) {
    return buildCalendarSyncMetadata({
      connected: true,
      observedAt,
      syncStatus: "not_mapped",
  ...
  if (requestedRoomIds.length > 0 && failedRooms > 0) {
    return buildCalendarSyncMetadata({
      connected: true,
      ok: false,
      observedAt,
      ...
      lastSyncedAt: latestSuccessAt,
      roomStatuses,
      error: "One or more rooms failed Channex provisioning. Retry room sync.",
      statusTitle: "Sync failed",
      statusDetail: "Showing saved calendar. One or more rooms failed Channex provisioning.",
    });
  }
