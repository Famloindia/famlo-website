...
export async function generatePlatformFeeInvoice(
  ...
  const [{ data: hostTaxDetails }, { data: hostGstProfile }] = await Promise.all([
    hostUserId
      ? supabase
          .from("host_tax_details")
          .select("pan_holder_name,verification_status")
    ...
    String(booking.host_id)
      ? supabase
          ...
          .eq("host_id", String(booking.host_id))
          .maybeSingle()
      : Promise.resolve({ data: null as JsonRecord | null }),
  ]);
  ...
  const artifact = buildPlatformFeeInvoiceArtifact({
    ...
    hostLegalName:
      ...
      "Host",
    hostGstin:
      asString((hostGstProfile as JsonRecord | null)?.gstin) ??
