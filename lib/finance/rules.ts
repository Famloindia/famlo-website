...
export async function resolveFinanceRules(
  ...
  if (input.productType === "host_stay") {
    const flatMarketplaceCommissionBps = toBpsFromPct(DEFAULT_COMMISSION_PCT);
    if (commissionRateBps !== flatMarketplaceCommissionBps) {
      warnings.push(`Normalized host_stay commission to flat ${DEFAULT_COMMISSION_PCT}% policy.`);
    }
    commissionRateBps = flatMarketplaceCommissionBps;
  }
