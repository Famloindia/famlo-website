...
export type AdminInvoiceRow = {
  id: string;
  kind: "guest_tax_invoice" | "platform_fee_invoice" | "credit_note" | "host_pro_invoice";
  number: string;
  bookingId: string;
  status: string;
  amount: number;
...
export async function loadAdminInvoiceRows(supabase: SupabaseClient): Promise<AdminInvoiceRow[]> {
  const [
    { data: guestInvoices, error: guestError },
    { data: platformInvoices, error: platformError },
    { data: creditNotes, error: creditError },
    { data: hostProInvoices, error: hostProError },
  ] =
    await Promise.all([
      ...
      supabase.from("host_pro_invoices").select("id,billing_order_id,invoice_number,status,total_paid,issued_at,created_at,email_status,whatsapp_status"),
    ]);
  if (guestError) throw guestError;
  if (platformError) throw platformError;
  if (creditError) throw creditError;
  if (hostProError) throw hostProError;
  ...
  return [
    ...
    ...((hostProInvoices ?? []) as JsonRecord[]).map((row) => ({
      id: asString(row.id) ?? "",
      kind: "host_pro_invoice" as const,
      number: asString(row.invoice_number) ?? "",
      bookingId: asString(row.billing_order_id) ?? "",
      status: asString(row.status) ?? "",
      amount: asNumber(row.total_paid),
      issuedAt: asString(row.issued_at) ?? asString(row.created_at),
      downloadHref: `/api/admin/finance/invoices/${asString(row.id) ?? ""}/download`,
      emailStatus: asString(row.email_status) ?? deliveryByArtifactId.get(asString(row.id) ?? "") ?? null,
      whatsappStatus: asString(row.whatsapp_status) ?? null,
    })),
