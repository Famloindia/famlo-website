...
export async function createHostReelUploadTarget(input: {
  draftId: string;
  fileName: string;
  mimeType: string;
  sizeBytes: number;
  ...
  assertUploadConfig({
    uploadFolder: "host-reels",
    keyPrefix: `host-reels/${input.draftId}/`,
  ...
  const storageKey = `host-reels/${input.draftId}/${Date.now()}-${randomUUID()}.${extension}`;
