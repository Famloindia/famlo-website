...
export type ChannexUpdateRoomTypeOccupancyInput = {
  roomTypeId: string;
  propertyId: string;
  title: string;
  countOfRooms: number;
...
export type ChannexUpdateRoomTypeOccupancyResult = {
  ok: boolean;
  environment: ChannexEnvironment;
  endpoint: string;
  httpStatus: number | null;
...
export type ChannexUpdateRatePlanOccupancyInput = {
  ratePlanId: string;
  title: string;
  propertyId: string;
  roomTypeId: string;
...
export type ChannexUpdateRatePlanOccupancyResult = {
  ok: boolean;
  environment: ChannexEnvironment;
  endpoint: string;
  httpStatus: number | null;
...
export type ChannexBookingCrsUpdateResult = {
  ok: boolean;
  environment: ChannexEnvironment;
  endpoint: string;
  httpStatus: number | null;
...
export async function updateChannexRoomTypeOccupancy(
  input: ChannexUpdateRoomTypeOccupancyInput
): Promise<ChannexUpdateRoomTypeOccupancyResult> {
  const environment = loadEnvironment();
  const summary = getChannexConfigSummary();
  const endpointPath = `/api/v1/room_types/${encodeURIComponent(input.roomTypeId)}`;
  ...
  const current = await getChannexObjectJson(endpointPath);
  if (!current.ok) {
    return {
      ok: false,
      ...
      message: `Unable to load current Channex room type before update. ${current.message}`,
      externalRoomTypeId: input.roomTypeId,
      rawValidation: current.rawValidation,
    };
  }

  const attributes =
    current.data &&
    typeof current.data.attributes === "object" &&
    current.data.attributes &&
    !Array.isArray(current.data.attributes)
      ? (current.data.attributes as Record<string, unknown>)
      : {};
  const existingContent =
    attributes.content && typeof attributes.content === "object" && !Array.isArray(attributes.content)
      ? ({ ...(attributes.content as Record<string, unknown>) } as Record<string, unknown>)
      : null;
  const nextDescription = trimOrNull(input.description) ?? trimOrNull(typeof existingContent?.description === "string" ? existingContent.description : null);
  ...
  const payload = {
    room_type: compactObject({
      property_id: input.propertyId,
      title: trimOrNull(input.title) ?? trimOrNull(typeof attributes.title === "string" ? attributes.title : null),
      count_of_rooms: Math.max(
        1,
        Number(
          typeof attributes.count_of_rooms === "number"
            ? attributes.count_of_rooms
            : typeof attributes.count_of_rooms === "string"
              ? attributes.count_of_rooms
              : input.countOfRooms
        ) || input.countOfRooms
      ),
      occ_adults: Math.max(1, input.occAdults),
      occ_children: Math.max(0, input.occChildren),
      occ_infants: Math.max(0, input.occInfants),
      default_occupancy: Math.min(Math.max(1, input.defaultOccupancy), Math.max(1, input.occAdults)),
      room_kind: input.roomKind ?? trimOrNull(typeof attributes.room_kind === "string" ? attributes.room_kind : null) ?? "room",
      capacity:
        typeof attributes.capacity === "number"
          ? attributes.capacity
          : typeof attributes.capacity === "string" && attributes.capacity.trim().length > 0
            ? Number(attributes.capacity)
            : undefined,
      facilities: Array.isArray(attributes.facilities) ? attributes.facilities : undefined,
      content,
    }),
  };
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return {
        ...
        httpStatus: response.status,
        message: errorTitle
          ? `Channex room type occupancy update failed: ${errorTitle}.`
          : `Channex room type occupancy update failed with HTTP ${response.status}.`,
        externalRoomTypeId: input.roomTypeId,
        rawValidation: errors,
      };
    }
    ...
    return {
      ...
      endpoint: endpointPath,
      httpStatus: response.status,
      message: `Channex room type occupancy updated successfully for ${input.roomTypeId}.`,
      externalRoomTypeId: input.roomTypeId,
      rawValidation: null,
    };
  } catch (error) {
    return {
      ...
      message:
        error instanceof Error
          ? `Channex room type occupancy update failed: ${error.message}`
          : "Channex room type occupancy update failed.",
      externalRoomTypeId: input.roomTypeId,
      rawValidation: null,
    };
  }
...
export async function updateChannexRatePlanOccupancy(
  input: ChannexUpdateRatePlanOccupancyInput
): Promise<ChannexUpdateRatePlanOccupancyResult> {
  const environment = loadEnvironment();
  const summary = getChannexConfigSummary();
  const endpointPath = `/api/v1/rate_plans/${encodeURIComponent(input.ratePlanId)}`;
  const endpoint = `${resolveBaseUrl(environment)}${endpointPath}`;
  const apiKey = loadApiKey(environment);
  ...
  if (!current.ok) {
    return {
      ...
      endpoint: current.endpoint,
      httpStatus: current.httpStatus,
      message: `Unable to load current Channex rate plan before update. ${current.message}`,
      externalRatePlanId: input.ratePlanId,
      rawValidation: current.rawValidation,
    };
  }
  ...
  const attributes =
    ...
    !Array.isArray(current.data.attributes)
      ? (current.data.attributes as Record<string, unknown>)
      : {};
  const currentOptions = Array.isArray(attributes.options) ? attributes.options : [];
  const options =
    currentOptions.length > 0
      ? currentOptions.map((option, index) => {
          const optionRecord =
            option && typeof option === "object" && !Array.isArray(option)
              ? ({ ...(option as Record<string, unknown>) } as Record<string, unknown>)
              : {};
          const isPrimary = optionRecord.is_primary === true || (index === 0 && !currentOptions.some((entry) => {
            return Boolean(entry && typeof entry === "object" && !Array.isArray(entry) && (entry as Record<string, unknown>).is_primary === true);
          }));
          return isPrimary
            ? {
                ...optionRecord,
                occupancy: Math.max(1, input.occupancy),
              }
            : optionRecord;
        })
      : [
  ...
  const payload = {
    rate_plan: compactObject({
      title: trimOrNull(input.title) ?? trimOrNull(typeof attributes.title === "string" ? attributes.title : null),
      property_id: input.propertyId,
      room_type_id: input.roomTypeId,
      options,
      currency: trimOrNull(input.currency) ?? trimOrNull(typeof attributes.currency === "string" ? attributes.currency : null),
      sell_mode: trimOrNull(typeof attributes.sell_mode === "string" ? attributes.sell_mode : null) ?? "per_room",
      rate_mode: trimOrNull(typeof attributes.rate_mode === "string" ? attributes.rate_mode : null) ?? "manual",
      meal_type: trimOrNull(input.mealType) ?? trimOrNull(typeof attributes.meal_type === "string" ? attributes.meal_type : null),
      inherit_rate: attributes.inherit_rate,
      inherit_closed_to_arrival: attributes.inherit_closed_to_arrival,
      inherit_closed_to_departure: attributes.inherit_closed_to_departure,
      inherit_stop_sell: attributes.inherit_stop_sell,
      inherit_min_stay_arrival: attributes.inherit_min_stay_arrival,
      inherit_min_stay_through: attributes.inherit_min_stay_through,
      inherit_max_stay: attributes.inherit_max_stay,
      inherit_max_sell: attributes.inherit_max_sell,
      inherit_max_availability: attributes.inherit_max_availability,
      inherit_availability_offset: attributes.inherit_availability_offset,
    }),
  };
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return {
        ...
        httpStatus: response.status,
        message: errorTitle
          ? `Channex rate plan occupancy update failed: ${errorTitle}.`
          : `Channex rate plan occupancy update failed with HTTP ${response.status}.`,
        externalRatePlanId: input.ratePlanId,
        rawValidation: errors,
      };
    }
    ...
    return {
      ...
      endpoint: endpointPath,
      httpStatus: response.status,
      message: `Channex rate plan occupancy updated successfully for ${input.ratePlanId}.`,
      externalRatePlanId: input.ratePlanId,
      rawValidation: null,
    };
  } catch (error) {
    return {
      ...
      message:
        error instanceof Error
          ? `Channex rate plan occupancy update failed: ${error.message}`
          : "Channex rate plan occupancy update failed.",
      externalRatePlanId: input.ratePlanId,
      rawValidation: null,
    };
  }
...
export async function fetchChannexRestrictionsSnapshot(input: {
  ...
  params.set(
    "filter[restrictions]",
    "rate,stop_sell,closed_to_arrival,closed_to_departure,min_stay_through,min_stay_arrival,max_stay"
  );
...
export async function updateChannexBookingViaCrs(input: {
  bookingId: string;
  booking: Record<string, unknown>;
}): Promise<ChannexBookingCrsUpdateResult> {
  const environment = loadEnvironment();
  const summary = getChannexConfigSummary();
  const endpointPath = `/api/v1/bookings/${encodeURIComponent(input.bookingId)}`;
  const endpoint = `${resolveBaseUrl(environment)}${endpointPath}`;
  const apiKey = loadApiKey(environment);
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return {
        ...
        httpStatus: response.status,
        message: errorTitle
          ? `Channex booking CRS update failed: ${errorTitle}.`
          : `Channex booking CRS update failed with HTTP ${response.status}.`,
        bookingId: null,
        uniqueId: null,
        revisionId: null,
        status: null,
    ...
    const bookingId =
      trimOrNull(typeof data?.id === "string" ? data.id : null) ??
      trimOrNull(typeof attributes?.booking_id === "string" ? attributes.booking_id : null) ??
      trimOrNull(typeof attributes?.id === "string" ? attributes.id : null);
    const uniqueId = trimOrNull(typeof attributes?.unique_id === "string" ? attributes.unique_id : null);
    const revisionId = trimOrNull(typeof attributes?.revision_id === "string" ? attributes.revision_id : null);
    const status = trimOrNull(typeof attributes?.status === "string" ? attributes.status : null);
    ...
    return {
      ...
      message:
        typeof meta?.message === "string"
          ? meta.message
          : "Channex booking CRS update completed successfully.",
      bookingId,
      uniqueId,
      revisionId,
      status,
    ...
    return {
      ...
      message:
        error instanceof Error
          ? `Channex booking CRS update failed: ${error.message}`
          : "Channex booking CRS update failed.",
      bookingId: null,
      uniqueId: null,
      revisionId: null,
      status: null,
