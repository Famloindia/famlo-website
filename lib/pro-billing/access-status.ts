...
export function deriveProAccessStatus(
  ...
  if (explicitStatus === "cancelled") {
    if (periodMillis !== null && nowMillis <= periodMillis) {
      return {
        ...
        currentPeriodEnd,
        graceUntil,
        reason: "cancelled_pending_period_end",
      };
    }
  ...
  if (explicitStatus === "payment_failed" || explicitStatus === "halted") {
    ...
    if (periodMillis !== null && nowMillis <= periodMillis) {
      return {
        ...
        currentPeriodEnd,
        graceUntil,
        reason: explicitStatus === "halted" ? "subscription_halted_paid_period" : "payment_failed_paid_period",
      };
    }
    ...
    if (graceMillis !== null && nowMillis <= graceMillis) {
      return {
        ...
        currentPeriodEnd,
        graceUntil,
        reason: explicitStatus === "halted" ? "subscription_halted_in_grace" : "payment_failed_in_grace",
      };
    }
    ...
    return {
      ...
      currentPeriodEnd,
      graceUntil,
      reason: explicitStatus === "halted" ? "subscription_halted_grace_expired" : "payment_failed_grace_expired",
    };
  }
  ...
  return {
    ...
    currentPeriodEnd,
    graceUntil,
    reason: explicitStatus === "inactive" ? "subscription_inactive" : "status_unavailable",
  };
}
...
export function canUseProFeature(input: {
  ...
  return deriveProAccessStatus(input.subscription ?? null, { now: input.now }).allowed;
}
