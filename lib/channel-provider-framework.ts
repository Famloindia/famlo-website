...
import { isChannexRoomSyncJob, processChannexRoomSyncJob } from "@/lib/channex-room-sync-jobs";
...
async function deferChannexAriJobForRateLimit(
  ...
  await updateChannelSyncJobOrThrow(supabase, input.jobId, {
    status: "retrying",
...
async function updateChannelSyncJobOrThrow(
  supabase: SupabaseClient,
  jobId: string,
  patch: JsonRecord
): Promise<void> {
  const { error } = await supabase
    ...
    .eq("id", jobId);
  if (error) {
    throw new Error(`Failed to persist channel sync job ${jobId}: ${error.message}`);
  }
}
...
export async function processDueChannelSyncJobs(
  ...
  input?: {
    ...
    workerId?: string;
    dependencies?: {
      processChannexAriSyncJob?: typeof processChannexAriSyncJob;
      executeChannelProviderOperation?: typeof executeChannelProviderOperation;
    };
  }
): Promise<{
  processed: number;
  ...
  const workerId = input?.workerId ?? "channel-sync-worker";
  const processAriJob = input?.dependencies?.processChannexAriSyncJob ?? processChannexAriSyncJob;
  const executeProviderOperation =
    input?.dependencies?.executeChannelProviderOperation ?? executeChannelProviderOperation;
  ...
  for (const job of (jobs ?? []) as JsonRecord[]) {
    ...
    const createdAtMs = Date.parse(asString(job.created_at) ?? "");
    const runAfterMs = Date.parse(asString(job.run_after) ?? "");
    const queueEligibleAtMs = Number.isFinite(runAfterMs) ? runAfterMs : createdAtMs;
    const queueWaitMs = Number.isFinite(queueEligibleAtMs) ? Math.max(0, claimedAtMs - queueEligibleAtMs) : null;
    const jobAgeMs = Number.isFinite(createdAtMs) ? Math.max(0, claimedAtMs - createdAtMs) : null;
    const timingMetrics = {
      worker_id: workerId,
    ...
    if (queueWaitMs != null) queueWaitMsValues.push(queueWaitMs);
    if (jobAgeMs != null) jobAgeMsValues.push(jobAgeMs);
    ...
    try {
      if (isChannexAriJobType(jobType)) {
        ...
        if (deferredForRateLimit) {
          failed += 1;
          results.push({ job_id: jobId, ok: false, deferred: true, reason: "channex_ari_rate_limit", ...timingMetrics });
          continue;
        }

        const ariResult = await processAriJob(supabase, job);
        ...
        await updateChannelSyncJobOrThrow(supabase, jobId, {
          status: nextStatus,
          result: {
            ...ariResult.result,
        ...
        results.push({ job_id: jobId, ok: ariResult.ok, result: ariResult.result, task_ids: ariResult.taskIds, ...timingMetrics });
        continue;
      }

      if (isChannexRoomSyncJob(job)) {
        const roomResult = await processChannexRoomSyncJob(supabase, job);
        const nextStatus = roomResult.ok
          ? "succeeded"
          : roomResult.retryable
            ? attempts >= maxAttempts
              ? "dead_lettered"
              : "retrying"
            : "failed";
        ...
        await updateChannelSyncJobOrThrow(supabase, jobId, {
          ...
          last_error: roomResult.ok ? null : roomResult.message,
          dead_lettered_at: nextStatus === "dead_lettered" ? new Date().toISOString() : null,
          completed_at: nextStatus === "succeeded" || nextStatus === "failed" ? new Date().toISOString() : null,
          processed_at: roomResult.ok ? new Date().toISOString() : null,
          run_after:
            nextStatus === "retrying"
              ? new Date(Date.now() + Math.min(60, attempts * 10) * 60_000).toISOString()
              : new Date().toISOString(),
          updated_at: new Date().toISOString(),
        });

        if (roomResult.ok) succeeded += 1;
        else if (nextStatus === "dead_lettered") deadLettered += 1;
        else failed += 1;
        results.push({ job_id: jobId, ok: roomResult.ok, result: roomResult.result, ...timingMetrics });
        continue;
      }
      ...
      const operationType: ChannelProviderOperationType =
        jobType === "provider_reconcile"
          ...
          : jobType === "diagnostic_check"
            ? "test_provider"
            : "refresh_provider";
      const result = await executeProviderOperation(supabase, {
        familyId,
        providerKey,
        operationType,
        actorRole: "system",
      ...
      results.push({ job_id: jobId, ok: result.ok, result, ...timingMetrics });
    } catch (error) {
      ...
      try {
        ...
        const persistMessage = persistError instanceof Error ? persistError.message : "Unknown persistence failure.";
        throw new Error(`Failed to record job failure for ${jobId}: ${message}. ${persistMessage}`);
      }
      if (dead) deadLettered += 1;
      else failed += 1;
      results.push({ job_id: jobId, ok: false, error: message, ...timingMetrics });
    }
  }
  const sortedQueueWaitMs = [...queueWaitMsValues].sort((left, right) => left - right);
  const percentileIndex = sortedQueueWaitMs.length > 0 ? Math.ceil(sortedQueueWaitMs.length * 0.95) - 1 : -1;
  ...
  return {
    ...
    metrics: {
      worker_id: workerId,
      claimed_count: ((jobs ?? []) as JsonRecord[]).length,
      processed_count: results.length,
      queue_wait_ms_avg:
        queueWaitMsValues.length > 0
          ? Math.round(queueWaitMsValues.reduce((sum, value) => sum + value, 0) / queueWaitMsValues.length)
          : null,
      queue_wait_ms_max: sortedQueueWaitMs.at(-1) ?? null,
      queue_wait_ms_p95: percentileIndex >= 0 ? sortedQueueWaitMs[percentileIndex] ?? null : null,
      job_age_ms_max: jobAgeMsValues.length > 0 ? Math.max(...jobAgeMsValues) : null,
    },
  };
}
