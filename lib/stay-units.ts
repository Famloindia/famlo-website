...
function asCsvStringArray(value: unknown): string[] {
...
function roomDraftIdentityMatches(room: JsonRecord, row: StayUnitRecord): boolean {
  ...
  const roomUnitKey = asNullableString(room.unitKey ?? room.unit_key);
  const roomName = asNullableString(room.roomName ?? room.name);

  return (
    (roomId != null && (roomId === row.id || roomId === row.unitKey)) ||
    (roomUnitKey != null && roomUnitKey === row.unitKey) ||
...
function buildComparableStayUnitSnapshot(row: StayUnitRecord): JsonRecord {
  return {
    ...
    toiletTypes: dedupeStrings(row.toiletTypes),
...
function preserveCanonicalRoomMedia(existingRows: StayUnitRecord[], desiredRows: JsonRecord[]): JsonRecord[] {
  return desiredRows.map((desiredRow) => {
    const desiredRecord = mapStayUnitRow(desiredRow);
    const existingRow = existingRows.find(
      (row) => row.unitKey === desiredRecord.unitKey || row.id === desiredRecord.id || row.name === desiredRecord.name
    );
    if (!existingRow) return desiredRow;

    return {
      ...desiredRow,
      photos: desiredRecord.photos.length > 0 ? desiredRecord.photos : existingRow.photos,
      locality_photos:
        desiredRecord.localityPhotos.length > 0 ? desiredRecord.localityPhotos : existingRow.localityPhotos,
    };
  });
}
...
export function mapStayUnitRow(row: JsonRecord): StayUnitRecord {
  return {
    ...
    toiletTypes: dedupeStrings([
      ...asCsvStringArray(row.toilet_types),
      ...asCsvStringArray(row.toilet_type),
    ]),
...
function buildRoomRowsFromApplication(
  ...
  const rows = rawRooms
    .map((room, index) => {
      ...
      const id =
        asNullableString(roomRow.unitKey) ??
        asNullableString(roomRow.unit_key) ??
        asNullableString(roomRow.id) ??
        `room-${index + 1}`;
      ...
      const toiletTypes = dedupeStrings([
        ...asCsvStringArray(roomRow.toiletTypes),
        ...asCsvStringArray(roomRow.toilet_types),
        ...asCsvStringArray(roomRow.toiletType),
        ...asCsvStringArray(roomRow.toilet_type),
      ]);
...
async function repairStayUnitsFromApprovedSource(
  ...
  const desiredRows = preserveCanonicalRoomMedia(
    existingRows,
    buildRoomRowsFromApplication(normalizeApplicationSource(roomSource), {
...
export async function syncPrimaryStayUnitForFamily(
  ...
  if (roomsFromApplication.length > 0) {
    ...
    for (const desiredRow of roomsFromApplication) {
      ...
      if (existingRow?.id) {
        const desiredRecord = mapStayUnitRow(desiredRow);
        syncedRooms.push(await updateStayUnitRowWithSchemaFallback(supabase, existingRow.id, {
          ...desiredRow,
          photos: desiredRecord.photos.length > 0 ? desiredRecord.photos : existingRow.photos,
          locality_photos:
            desiredRecord.localityPhotos.length > 0 ? desiredRecord.localityPhotos : existingRow.localityPhotos,
