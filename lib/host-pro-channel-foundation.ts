...
import { redactChannelCredentialMetadata } from "@/lib/channel-credentials";
...
export async function loadHostProChannelFoundation(
  ...
  const includeSyncJobs = options?.includeSyncJobs !== false;
  ...
  const [
    ...
    includeSyncJobs
      ? supabase
          ...
          .select("id,family_id,provider_code,job_type,status,last_error,attempts,max_attempts,run_after,processed_at,created_at,updated_at")
          .eq("family_id", normalizedFamilyId)
          .order("created_at", { ascending: false })
          .limit(50)
      : Promise.resolve({ data: [], error: null }),
  ...
  return {
    providers: (providersResult.data ?? []).map((row) => ({
      ...
      metadata: redactChannelCredentialMetadata(row.metadata),
    ...
    syncJobs: (syncJobsResult.data ?? []).map((row) => ({
      id: asString(row.id) ?? "",
      familyId: asString(row.family_id) ?? normalizedFamilyId,
      providerCode: asString(row.provider_code) ?? "",
      jobType: asString(row.job_type) ?? "unknown",
      status: asString(row.status) ?? "unknown",
      lastError: asString(row.last_error),
      attempts: Math.max(0, asNumber(row.attempts, 0)),
      maxAttempts: Math.max(0, asNumber(row.max_attempts, 0)),
