...
import { enumerateDateRange, asString, getStayNightDateRange, type JsonRecord } from "@/lib/platform-utils";
  ...
  const bookingDerived =
    input.ownerType === "host" || input.ownerType === "stay_unit"
      ? [...bookingRows.values()].flatMap((row) => {
          ...
          const rawStartDate = normalizedDate(asString(row.start_date)) ?? input.from;
          const rawEndDate = normalizedDate(asString(row.end_date)) ?? rawStartDate;
          const stayNightRange = getStayNightDateRange(rawStartDate, rawEndDate);
          if (!stayNightRange) {
            return [];
          }
          const startDate = stayNightRange.from;
          const endDate = stayNightRange.to;
