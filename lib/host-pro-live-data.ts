...
async function loadRoomsForFamily(supabase: SupabaseClient, familyId: string): Promise<RoomSummary[]> {
  const { data, error } = await supabase
    .from("stay_units_v2")
    .select("id,name,unit_type,price_full_day,legacy_family_id,is_active")
  ...
  return ((data ?? []) as Array<Record<string, unknown>>)
    ...
    .map((row) => ({
      ...
      priceFullday: asNumber(row.price_full_day),
    }))
    .filter((row) => row.id);
}
...
export async function loadLiveProBookingsSnapshot(
  ...
  input: { familyId: string; globalCommission?: number }
  ...
  const { data: bookingRowsData, error: bookingRowsError } = await supabase
    .from("bookings_v2")
    .select("id,status,payment_status,total_price,partner_payout_amount,start_date,end_date,created_at,guests_count,stay_unit_id,pricing_snapshot,users!user_id(name,email)")
    .in("host_id", hostIds)
    .order("start_date", { ascending: false })
    .limit(120);
  if (bookingRowsError) throw bookingRowsError;
  const bookingRows = (bookingRowsData ?? []) as Array<Record<string, unknown>>;
  ...
  const persisted: LiveProBookingSummary[] = bookingRows.flatMap((row) => {
    ...
    const matchedRevision = bookingRevisionByLinkedBookingId.get(bookingId) ?? (externalBookingId ? bookingRevisionByExternalBookingId.get(externalBookingId) : null) ?? null;
    ...
    const latestPayout = (payoutRowsByBookingId.get(bookingId) ?? []).sort((a, b) => String(b.processed_at ?? b.created_at ?? "").localeCompare(String(a.processed_at ?? a.created_at ?? "")))[0] ?? null;
    const payoutAmountValue = asNumberOrNull(settlementLine?.amount);
    const platformFeeAmount = asNumber(platformInvoice?.total_amount) || asNumber(pricingSnapshot.platform_fee) || asNumber(financeContract.platform_fee) || asNumber(pricingSnapshot.famlo_platform_fee_incl_gst);
    const taxAmount = asNumber(pricingSnapshot.platform_fee_tax) || asNumber(financeContract.gst_on_platform_fee) || asNumber(pricingSnapshot.famlo_platform_fee_gst) || asNumber(pricingSnapshot.tax_amount);
    const refundAdjustmentAmount = asNumber(asObject(settlementLine?.metadata).refund_adjustment_amount) || asNumber(folio?.refund_total_amount) || creditNoteAmount || asNumber(financeSnapshot.refund_adjustments) || asNumber(pricingSnapshot.refund_adjustment);
    ...
    const otaCommissionAmount = isOta && amountValue != null && netPayoutAmount != null
      ...
      : null;
    const payoutStatus = asString(payoutExecution?.status) ?? asString(settlement?.status) ?? asString(latestPayout?.status);
    const payoutPaidAt = asString(payoutExecution?.processed_at) ?? asString(settlement?.paid_at) ?? asString(latestPayout?.processed_at) ?? asString(latestPayout?.created_at);
    ...
    const settlementEligible = Boolean(folioMetadata.is_settlement_eligible === true || settlementLine || settlement);
    const famloPayoutEligible = paymentCollectMode === "FAMLO_COLLECT" && Boolean(settlementLine);
    const revenueDate = isCompletedRevenueStatus(reservationStatus) || isCompletedRevenueStatus(row.status) || Boolean(settlementLine) ? checkoutDate : null;
    ...
    return [{
      ...
      sourceLabel: isOta ? `${matchedRevision?.otaName ?? "OTA"} / Channex` : sourceChannel === "pms_manual" ? "Famlo PMS" : "Famlo Direct",
      sourceCategory,
      paymentCollectMode,
      famloPayoutEligible,
      settlementEligible,
      ...
      payoutExecutionStatus: asString(payoutExecution?.status),
      complianceBlocked: !(panVerified && payoutAccountActive),
      payoutStatus,
      payoutPaidAt,
      estimatedPayoutDate: payoutStatus === "paid" ? payoutPaidAt : endDate,
      famloRevenueAmount: asNumberOrNull(platformInvoice?.total_amount) ?? asNumberOrNull(financeContract.famlo_net_revenue) ?? asNumberOrNull(pricingSnapshot.famlo_platform_fee_taxable),
  ...
  const reviewOnly = revisions.flatMap((revision) => {
...
export async function loadLiveCalendarSnapshot(
  ...
  if (hostIds.length > 0) {
    let query = supabase
      .from("bookings_v2")
      .select("id,status,payment_status,total_price,start_date,end_date,stay_unit_id,pricing_snapshot,users!user_id(name)")
      .in("host_id", hostIds)
    ...
    if (!error) bookingRowsForCalendar = (data ?? []) as Array<Record<string, unknown>>;
  }
  ...
  for (const room of filteredRooms) {
    const projectedDays = await ensureProjectedInventory(supabase, {
    ...
    roomProjectedRates.set(room.id, rateMap);
    roomProjectedAvailability.set(room.id, availabilityMap);
    roomManualBlockDates.set(room.id, blockSet);
    ...
    roomDailyRateOverrides.set(room.id, rateOverrides);
  }
