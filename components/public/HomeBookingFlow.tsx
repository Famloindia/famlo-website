...
export function HomeBookingFlow({ home, existingBookings = [], stayUnits = [] }: Readonly<HomeBookingFlowProps>): React.JSX.Element {
  ...
  const [selectedStayUnitId, setSelectedStayUnitId] = useState<string>(requestedStayUnitId ?? activeStayUnits[0]?.id ?? stayUnits[0]?.id ?? "");
  const resolvedSelectedStayUnitId = useMemo(() => {
    if (requestedStayUnitId && stayUnits.some((unit) => unit.id === requestedStayUnitId && unit.isActive)) {
      return requestedStayUnitId;
    }

    if (selectedStayUnitId && stayUnits.some((unit) => unit.id === selectedStayUnitId && unit.isActive)) {
      return selectedStayUnitId;
    }

    return activeStayUnits[0]?.id ?? "";
  }, [activeStayUnits, requestedStayUnitId, selectedStayUnitId, stayUnits]);
  const selectedStayUnit = useMemo(
    () => activeStayUnits.find((unit) => unit.id === resolvedSelectedStayUnitId) ?? activeStayUnits[0] ?? null,
    [activeStayUnits, resolvedSelectedStayUnitId]
  );
  const effectiveBlockedDates = useMemo(
    () => Array.from(new Set(selectedStayUnit?.blockedDates ?? [])),
    [selectedStayUnit?.blockedDates]
  );
  ...
  const [selectedQuarterId, setSelectedQuarterId] = useState<string>(
    () => (requestedQuarter && quarterOptions.some((quarter) => quarter.id === requestedQuarter) ? requestedQuarter : quarterOptions[0]?.id ?? "")
  );
  const [dateFrom, setDateFrom] = useState(() => requestedDate || getToday());
  const [dateTo, setDateTo] = useState(() => requestedDateTo || requestedDate || getToday());
  const dateFromRef = useRef<HTMLInputElement>(null);
  const dateToRef = useRef<HTMLInputElement>(null);
  const [guestCount, setGuestCount] = useState(() => (Number.isFinite(requestedGuests) && requestedGuests > 0 ? requestedGuests : 1));
  ...
  const resolvedSelectedQuarterId = useMemo(() => {
    if (selectedQuarterId && quarterOptions.some((quarter) => quarter.id === selectedQuarterId)) {
      return selectedQuarterId;
    }

    return quarterOptions[0]?.id ?? "";
  }, [quarterOptions, selectedQuarterId]);
  const selectedQuarter = useMemo(
    () => quarterOptions.find((quarter) => quarter.id === resolvedSelectedQuarterId) ?? null,
    [quarterOptions, resolvedSelectedQuarterId]
  );
...
Need help during your stay? Use the Famlo assistance path from your booking thread. If it is urgent, open the emergency support option in your booking dashboard and our team can help right away.`;
      ...
      if (payload.bookingId) {
        ...
        if (!paymentIntentPayload) {
          ...
          checkout.on?.("payment.failed", (failureResponse: RazorpayFailureResponse) => {
            void releasePendingBooking(order.bookingId).catch((cancelError) => {
              console.error("[home-booking-flow] release_pending_booking_failed", cancelError);
            });
            setBookingReceipt(null);
          ...
          if (result && "error" in result && result.error) {
            setBookingError("Your payment was not completed. You can try again while Famlo waits for the signed payment update.");
            return;
          }
          ...
          setSuccessMessage(
            "Payment confirmation is pending. Famlo will update the booking after the signed confirmation arrives."
          );
          setStep("confirm");
          return;
        } else {
            ...
            <>
              ...
              <section className="famlo-reveal-block">
                ...
                <div style={{ display: "grid", gap: 12 }}>
                  {stayUnits.map((unit) => {
                    const isSelected = unit.id === resolvedSelectedStayUnitId;
                      ...
                      >
                        <div style={{ minWidth: 0 }}>
                          ...
                          <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 10 }}>
                            ...
                            {unit.toiletTypes && unit.toiletTypes.length > 0 ? <span className="famlo-setup-badge">{unit.toiletTypes.join(", ")}</span> : null}
                            <span className="famlo-setup-badge">
                      ...
                      {guestFitStayUnits.length > 0 ? (
                        <div style={{ display: "grid", gap: 8, marginTop: 12 }}>
                          {guestFitStayUnits.slice(0, 3).map((unit) => {
                            const roomPrice = unit.priceFullday || unit.priceMorning || unit.priceAfternoon || unit.priceEvening;
                            const isCurrentRoom = unit.id === resolvedSelectedStayUnitId;
                            return (
                              <button
