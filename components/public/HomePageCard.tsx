...
import { getValidHomeRating } from "@/lib/public-destinations";
...
export function HomePageCard({ home, distance }: Readonly<{ home: HomeCardRecord; distance?: string }>): React.JSX.Element {
  ...
  const reviewLabel = home.totalReviews != null && home.totalReviews > 0 ? ` (${home.totalReviews})` : "";
      ...
      >
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "12px" }}>
          ...
          <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
            {rating != null ? (
              <Badge bg="rgba(0,0,0,0.36)" color="#fff">★ {rating.toFixed(1)}{reviewLabel}</Badge>
            ) : (
              <Badge bg="rgba(0,0,0,0.3)" color="#fff">NEW</Badge>
            )}
            {isOpen ? <Badge bg="rgba(5,150,105,0.9)" color="#fff">OPEN</Badge> : null}
            {home.superhost ? <Badge bg="rgba(251,191,36,0.9)" color="#111827">SUPERHOST</Badge> : null}
          </div>
        </div>
