  ...
  return (
    <div style={{ display: "grid", gap: "8px", minWidth: "220px" }}>
      ...
      <textarea
        ...
        placeholder="Required admin reason for this manual payout override"
        disabled={row.payoutStatus === "paid" || submitting}
        style={{
          borderRadius: "10px",
          border: "1px solid rgba(255,255,255,0.1)",
      ...
      <button
        onClick={markPaid}
        disabled={row.payoutStatus === "paid" || submitting || reason.trim().length < 10}
        style={{
          borderRadius: "10px",
          border: "none",
          background: row.payoutStatus === "paid" || reason.trim().length < 10 ? "rgba(255,255,255,0.08)" : "#166534",
          color: "white",
          fontWeight: 800,
          padding: "10px 12px",
          cursor: row.payoutStatus === "paid" || reason.trim().length < 10 ? "not-allowed" : "pointer",
        }}
      >
        {submitting ? "Saving..." : row.payoutStatus === "paid" ? "Already Paid" : "Mark Paid"}
      </button>
      <div style={{ fontSize: "11px", color: reason.trim().length >= 10 ? "#86efac" : "rgba(255,255,255,0.45)" }}>
        Explicit admin reason required, minimum 10 characters.
      </div>
...
export default function ManualPayoutDesk({
  rows,
  ...
  return (
    <div style={{ display: "grid", gap: "18px" }}>
      ...
      {executionDisabledReason ? (
        ...
        >
          Provider payout execution is disabled: {executionDisabledReason}
        </div>
      ) : null}
      ...
      >
        <input
          ...
          placeholder="Search partner, guest, booking"
          style={{
            borderRadius: "10px",
            border: "1px solid rgba(255,255,255,0.1)",
            background: "rgba(2,6,23,0.6)",
              ...
              {filteredRows.map((row) => (
                <tr key={row.payoutId} style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                  ...
                  <td style={{ padding: "14px 10px", color: "#cbd5e1", fontSize: "12px", lineHeight: 1.5 }}>
                    ...
                    <div style={{ color: "rgba(255,255,255,0.45)", marginTop: "4px" }}>Destination hidden in admin override desk</div>
                  </td>
