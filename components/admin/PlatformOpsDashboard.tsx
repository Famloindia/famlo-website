...
export default function PlatformOpsDashboard() {
  ...
  const cards = [
    ...
    { label: "Channel Ops", value: `${data.channelOpsReadiness.readinessPercent}%` },
    { label: "Channel Alerts", value: data.channelOpsReadiness.alerts.length },
  ...
  return (
    <div style={{ display: "grid", gap: "18px" }}>
      ...
      <div style={{ display: "grid", gap: "14px", gridTemplateColumns: "repeat(auto-fit,minmax(280px,1fr))" }}>
        ...
        <section style={{ background: "rgba(255,255,255,0.04)", borderRadius: "18px", padding: "18px", border: "1px solid rgba(255,255,255,0.08)" }}>
          <h2 style={{ margin: 0, color: "white", fontSize: "18px" }}>Famlo Pro Channel Ops</h2>
          <p style={{ color: "rgba(255,255,255,0.5)" }}>
            {data.channelOpsReadiness.severity.toUpperCase()} · queue {data.channelOpsReadiness.metrics.queueDepth ?? 0} · dead letters {data.channelOpsReadiness.metrics.deadLetteredJobs ?? 0} · unacked OTA {data.channelOpsReadiness.metrics.unackedAppliedRevisions ?? 0}
          </p>
          {data.channelOpsReadiness.alerts.slice(0, 3).map((alert) => (
            <p key={alert.id} style={{ color: alert.severity === "critical" ? "#fecaca" : "#fde68a" }}>
