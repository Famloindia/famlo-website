...
function BannersManager({ banners, adminId }: { banners: BannerRecord[]; adminId: string }) {
  ...
  const activePreviewIdx = previewIdx >= activeBanners.length ? 0 : previewIdx;
  ...
  const openBannerForm = () => {
    const nextSort = Math.max(0, ...sortedBanners.map((banner) => Number(banner.sort_order ?? 0))) + 1;
    setForm((current) => ({
      ...current,
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <button onClick={() => (showForm ? closeBannerForm() : openBannerForm())} style={{ ...btnBase, background: "#1A56DB", color: "#fff" }}>
        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "12px" }}>
            ...
            <div style={{ color: "rgba(255,255,255,0.4)", fontWeight: 700, fontSize: "12px" }}>
              Auto-swipes every 3s • {activePreviewIdx + 1}/{activeBanners.length}
            </div>
          </div>
          <div style={{
            width: "100%",
            ...
            <div style={{
              position: "absolute",
              inset: 0,
              backgroundImage: `url(${activeBanners[activePreviewIdx]?.image_url})`,
          ...
          <div style={{ display: "flex", gap: "8px", justifyContent: "center" }}>
            {activeBanners.map((_, i) => (
              <button
                ...
                style={{
                  width: i === activePreviewIdx ? "26px" : "8px",
                  height: "8px",
                  borderRadius: "999px",
                  border: "none",
                  background: i === activePreviewIdx ? "#fff" : "rgba(255,255,255,0.35)",
                  cursor: "pointer",
