...
function ApplicationCard({ app, actorId, actorRole }: { app: Application; actorId: string; actorRole: string }) {
  ...
  const [decision, setDecision] = useState<"approved" | "rejected" | "not_listed_allow_pro" | "block_completely" | null>(null);
  const [reason, setReason] = useState("");
  const [showFullDetail, setShowFullDetail] = useState(false);
  const [requestError, setRequestError] = useState<string | null>(null);

  const allChecked = checklist.every((item) => item.checked);
  const isFamilyApplication = (app.application_type ?? "family") === "family";
  ...
  const hostAgreementAccepted = Boolean(getDraftValue("platformAgreementAccepted", "hostAgreementAccepted"));
  ...
  const gstin = String(getDraftValue("gstin", "gstNumber") || "");
  const gstDeclarationAccepted = Boolean(getDraftValue("gstDeclarationAccepted"));
  const hostReelUrl = String(getDraftValue("hostReelPublicUrl", "host_reel_public_url") || "");
  ...
  const nearbyPlaces = getDraftArray("nearbyPlaces", "landmarks");
  ...
  const handleDecision = async (action: "approved" | "rejected" | "not_listed_allow_pro" | "block_completely") => {
  ...
  if (decision) {
    const approvedLike = decision === "approved" || decision === "not_listed_allow_pro";
    return (
      <div style={{ background: "white", borderRadius: "20px", padding: "32px", border: `2px solid ${approvedLike ? "#86efac" : "#fca5a5"}`, textAlign: "center" }}>
        {approvedLike ? <CheckCircle2 size={40} color="#16a34a" /> : <XCircle size={40} color="#dc2626" />}
        <div style={{ marginTop: "16px", fontWeight: 900, fontSize: "18px", color: approvedLike ? "#16a34a" : "#dc2626" }}>
          {app.full_name} — {decision === "not_listed_allow_pro" ? "Pro workspace allowed, not listed" : decision === "block_completely" ? "Blocked" : decision === "approved" ? "Approved" : "Rejected"}
        </div>
      </div>
    );
  }
    ...
    <div style={{ background: "white", borderRadius: "20px", overflow: "hidden", border: "1px solid #e2e8f0", boxShadow: "0 4px 24px rgba(0,0,0,0.06)" }}>
          ...
          {draft ? (
            <div style={{ display: "flex", flexWrap: "wrap", gap: "8px", marginTop: "10px" }}>
                {[
                  ...
                  { label: hostReelUrl ? "Host reel ready" : "Host reel missing", tone: hostReelUrl ? "#15803d" : "#64748b", background: hostReelUrl ? "#f0fdf4" : "#f8fafc" },
      ...
      {showFullDetail && (
        <div style={{ background: "#f8fafc", borderBottom: "1px solid #e2e8f0" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1px", background: "#e2e8f0" }}>
            ...
            <div style={{ padding: "24px", background: "#f8fafc" }}>
              <div style={{ fontSize: "11px", fontWeight: 900, color: "#64748b", textTransform: "uppercase", marginBottom: "16px", letterSpacing: "0.05em" }}>Host Reel</div>
              {hostReelUrl ? (
                <div style={{ background: "white", padding: "14px", borderRadius: "16px", border: "1px solid #e2e8f0", marginBottom: "16px" }}>
                  <video
                    src={hostReelUrl}
                    controls
                    playsInline
                ...
                <div style={{ fontSize: "13px", color: "#64748b", background: "white", padding: "14px", borderRadius: "12px", border: "1px dashed #cbd5e1", marginBottom: "16px" }}>
                  No reel uploaded.
                </div>
              )}
            ...
            <div style={{ padding: "24px", background: "#f8fafc" }}>
               ...
               <div style={{ background: "white", padding: "16px", borderRadius: "14px", border: "1px solid #e2e8f0" }}>
                 <div style={{ display: "grid", gap: "10px" }}>
                   ...
                   <div style={{ display: "flex", justifyContent: "space-between", gap: "12px" }}>
                     <span style={{ fontSize: "12px", color: "#64748b", fontWeight: 800 }}>GSTIN</span>
                     <span style={{ fontSize: "13px", color: "#0e2b57", fontWeight: 900 }}>
                       {gstin || "Not provided"}
                     </span>
                   </div>
      ...
      <div style={{ padding: "24px" }}>
        ...
        <div style={{ display: "flex", gap: "12px" }}>
          <button onClick={() => handleDecision("rejected")} disabled={submitting} style={{ flex: 1, padding: "14px", borderRadius: "12px", border: "2px solid #fed7aa", background: "white", color: "#c2410c", fontWeight: 900, fontSize: "14px", cursor: "pointer" }}>{submitting ? <Loader2 className="animate-spin" size={16} /> : isFamilyApplication ? "Reject Marketplace" : "Reject Application"}</button>
          {isFamilyApplication ? (
            <>
              <button onClick={() => handleDecision("block_completely")} disabled={submitting} style={{ flex: 1, padding: "14px", borderRadius: "12px", border: "2px solid #fca5a5", background: "white", color: "#dc2626", fontWeight: 900, fontSize: "14px", cursor: "pointer" }}>{submitting ? <Loader2 className="animate-spin" size={16} /> : "Block"}</button>
              <button onClick={() => handleDecision("not_listed_allow_pro")} disabled={submitting} style={{ flex: 2, padding: "14px", borderRadius: "12px", border: "2px solid #bfdbfe", background: "#eff6ff", color: "#1d4ed8", fontWeight: 900, fontSize: "14px", cursor: "pointer" }}>{submitting ? <Loader2 className="animate-spin" size={16} /> : "Allow Pro Only"}</button>
            </>
          ) : null}
          <button onClick={() => handleDecision("approved")} disabled={!allChecked || submitting} style={{ flex: 2, padding: "14px", borderRadius: "12px", border: "none", background: allChecked ? "#22c55e" : "#e2e8f0", color: allChecked ? "white" : "#94a3b8", fontWeight: 900, fontSize: "14px", cursor: allChecked ? "pointer" : "not-allowed", transition: "all 0.2s" }}>{submitting ? <Loader2 className="animate-spin" size={16} /> : allChecked ? (isFamilyApplication ? "Approve Marketplace" : "Approve Application") : `Complete ${checklist.length - checklist.filter(i => i.checked).length} more checks to Approve`}</button>
        </div>
      </div>
    </div>
  );
