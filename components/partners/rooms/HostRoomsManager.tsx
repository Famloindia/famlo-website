...
type UploadPhotosPayload = {
  ...
  photoUrls?: string[];
  unitId?: string | null;
  canonicalStayUnitId?: string | null;
  error?: string;
};
...
export default function HostRoomsManager({
  ...
  const proCreateBlockedMessage =
    proCreateStatus === "grace"
      ? "Famlo Pro room add-ons are blocked during grace. Renew your paid Pro plan first, then buy the Famlo Pro room add-on."
      : proCreateStatus === "payment_failed" || proCreateStatus === "halted"
        ? "Famlo Pro room add-ons are blocked until billing is active again. Fix or renew the Pro plan first."
        : proCreateStatus && proCreateStatus !== "active"
          ? "Only an active paid Famlo Pro period can create a new Famlo Pro room."
          : null;
  ...
  const [pendingPhotoUploads, setPendingPhotoUploads] = useState<Record<string, PendingRoomPhotoUpload[]>>({});
  const pendingPhotoUploadsRef = useRef<Record<string, PendingRoomPhotoUpload[]>>({});
  ...
  const roomDraftIdKey = roomDrafts.map((room) => room.id).join("|");
  const roomDraftIds = useMemo(() => (roomDraftIdKey ? roomDraftIdKey.split("|") : []), [roomDraftIdKey]);
  ...
  useEffect(() => {
    return () => {
      Object.values(pendingPhotoUploadsRef.current).forEach((uploads) => {
  ...
  useEffect(() => {
    ...
    if (createDraftId && roomDraftIds.includes(createDraftId)) {
      return;
    }

    const blankRoom = createBlankRoom(roomDraftIds.length === 0);
    setCreateDraftId(blankRoom.id);
    setRoomDrafts((current) => [...current, blankRoom]);
  }, [createDraftId, createMode, roomDraftIdKey, roomDraftIds]);
  ...
  const updateRoomField = (roomId: string, field: keyof RoomFormState, value: string | boolean): void => {
    setRoomDrafts((current) =>
      current.map((room) => {
        ...
        if (nextRoom.quarterEnabled && field === "quarterEnabled" && !nextRoom.priceAfternoon.trim()) {
  ...
  const clearPendingUploadsForRoom = (roomId: string): void => {
    setPendingPhotoUploads((current) => {
      const uploads = current[roomId] ?? [];
      uploads.forEach((upload) => URL.revokeObjectURL(upload.previewUrl));
      const next = { ...current };
      delete next[roomId];
      return next;
  ...
  const clearPendingUploadsForRoomKind = (roomId: string, kind: "room" | "locality"): void => {
    setPendingPhotoUploads((current) => {
      const uploads = current[roomId] ?? [];
      const completed = uploads.filter((upload) => upload.kind === kind);
      completed.forEach((upload) => URL.revokeObjectURL(upload.previewUrl));
      const remaining = uploads.filter((upload) => upload.kind !== kind);
      const next = { ...current };
    ...
    pendingPhotoUploadsRef.current = {
      ...pendingPhotoUploadsRef.current,
      [roomId]: (pendingPhotoUploadsRef.current[roomId] ?? []).filter((upload) => upload.kind !== kind),
    };
    if (pendingPhotoUploadsRef.current[roomId]?.length === 0) {
      delete pendingPhotoUploadsRef.current[roomId];
    }
  };

  const movePendingUploadsToRoom = (fromRoomId: string, toRoomId: string): void => {
    if (fromRoomId === toRoomId) return;
    setPendingPhotoUploads((current) => {
      const uploads = current[fromRoomId] ?? [];
      if (uploads.length === 0) return current;
      const next = { ...current };
      delete next[fromRoomId];
      next[toRoomId] = [...(next[toRoomId] ?? []), ...uploads];
      return next;
    });
  };

  const removePendingUpload = (roomId: string, uploadId: string): void => {
    setPendingPhotoUploads((current) => {
      const uploads = current[roomId] ?? [];
      const removed = uploads.find((upload) => upload.id === uploadId);
      if (removed) URL.revokeObjectURL(removed.previewUrl);
      const remaining = uploads.filter((upload) => upload.id !== uploadId);
      const next = { ...current };
      if (remaining.length > 0) {
        next[roomId] = remaining;
      } else {
  ...
  const stagePhotoUploads = (roomId: string, files: File[], kind: "room" | "locality"): void => {
    ...
    setPendingPhotoUploads((current) => ({
      ...current,
      [roomId]: [...(current[roomId] ?? []), ...uploads],
    }));
    setRoomsMessage(`${files.length} ${kind === "room" ? "room" : "locality"} photo${files.length === 1 ? "" : "s"} selected. They will upload after this room is saved.`);
  };

  const uploadRoomPhotoFiles = async (
    room: RoomFormState,
    files: File[],
    kind: "room" | "locality",
    identity?: { unitId?: string | null; clientId?: string | null }
  ): Promise<UploadPhotosPayload> => {
    const optimizedFiles = await compressImageListForUpload(files);
    const formData = new FormData();
    formData.append("familyId", familyId);
    formData.append("unitId", identity?.unitId ?? room.id);
    if (room.unitKey) formData.append("unitKey", room.unitKey);
    if (room.name) formData.append("roomName", room.name);
    if (identity?.clientId) formData.append("clientId", identity.clientId);
    ...
    const payload = (await response.json()) as UploadPhotosPayload;
  ...
  const uploadPendingPhotosForRoom = async (
    room: RoomFormState,
    identity?: { unitId?: string | null; clientId?: string | null }
  ): Promise<{ uploaded: boolean; stayUnit?: StayUnitRecord }> => {
    const uploads = pendingPhotoUploadsRef.current[room.id] ?? [];
    if (uploads.length === 0) return { uploaded: false };

    let latestStayUnit: StayUnitRecord | undefined;
    for (const kind of ["room", "locality"] as const) {
      const files = uploads.filter((upload) => upload.kind === kind).map((upload) => upload.file);
      if (files.length === 0) continue;
      const payload = await uploadRoomPhotoFiles(room, files, kind, identity);
      latestStayUnit = payload.stayUnit;
      clearPendingUploadsForRoomKind(room.id, kind);
    }

    return { uploaded: true, stayUnit: latestStayUnit };
  };

  const addRoom = (): void => {
    if (proCreateBlockedMessage) {
      setRoomsMessage(proCreateBlockedMessage);
      return;
    }
  ...
  const saveRoom = async (room: RoomFormState, addonOrderId?: string): Promise<void> => {
    ...
    async function prepareAddonCheckout(): Promise<AddonCheckoutPayload> {
      const response = await fetch("/api/host/pro/addons/checkout", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ familyId, addonType: "room" }),
      });
      const payload = (await response.json()) as AddonCheckoutPayload;
      if (!response.ok) {
        throw new Error(payload.message ?? payload.error ?? "Failed to prepare room add-on checkout.");
      }
      return payload;
    }
    ...
    setRoomsMessage(null);
    try {
      if (room.id.startsWith("temp-") && proCreateBlockedMessage) {
        throw new Error(proCreateBlockedMessage);
      }
      ...
      const normalizedLng = room.lng.trim().length > 0 ? Number(room.lng) : null;
	      ...
	      const response = await fetch("/api/host/stay-units", {
	        ...
	        body: JSON.stringify({
	          familyId,
	          clientId: room.id,
	          operation: room.id.startsWith("temp-") ? "create" : "upsert",
	          addonOrderId,
	          unit: {
	      ...
	      const payload = (await response.json()) as {
	        ...
	        channexProvisioning?: {
	          ok?: boolean;
	          status?: "queued" | "not_mapped" | "failed" | string;
	          message?: string;
	          queuedJobIds?: string[];
	        } | null;
	        error?: string;
      ...
      if (response.status === 402 && payload.addonPaymentRequired) {
        if (payload.paymentExecutionEnabled === false) {
          throw new Error(payload.message ?? payload.error ?? "Payment execution is disabled in this environment.");
        }
        if (!confirmRoomAddonPayment(payload)) {
          throw new Error("Room add-on payment is required before this room can be created.");
        }
        const checkoutPayload = await prepareAddonCheckout();
        const paidAddonOrderId = await openAddonCheckout(checkoutPayload);
        await saveRoom(room, paidAddonOrderId);
        return;
      }
      if (!response.ok || !payload.stayUnit) {
        throw new Error(payload.message ?? payload.error ?? "Failed to save room.");
      }

	      const previousId = payload.clientId ?? room.id;
	      const canonicalRoomId = payload.canonicalStayUnitId ?? payload.unitId ?? payload.stayUnit.id;
	      const savedRoom = roomToForm(payload.stayUnit as StayUnitRecord);
	      ...
	      const pendingUploads = pendingPhotoUploadsRef.current[previousId] ?? [];
	      let photoStatus = "";
	      if (pendingUploads.length > 0) {
	        if (!canonicalRoomId) {
	          throw new Error("Room saved, but the canonical room ID was missing for photo upload.");
	        }
	        setRoomsMessage("Room saved. Photos uploading...");
	        const uploadRoom = { ...savedRoom, id: previousId };
	        try {
	          const uploadResult = await uploadPendingPhotosForRoom(uploadRoom, {
	            unitId: canonicalRoomId,
	            clientId: previousId,
	          });
	          if (uploadResult.stayUnit) {
	            const uploadedRoom = roomToForm(uploadResult.stayUnit);
	            setRoomDrafts((current) =>
	              current.map((item) => (item.id === canonicalRoomId || item.id === previousId ? uploadedRoom : item))
	            );
	          }
	          photoStatus = uploadResult.uploaded ? " Photos saved." : "";
	        } catch (uploadError) {
	          movePendingUploadsToRoom(previousId, canonicalRoomId);
	          photoStatus = ` Photo upload failed: ${uploadError instanceof Error ? uploadError.message : "retry from Room photos"}.`;
	        }
	      }
	      const channexStatus = payload.channexProvisioning?.status ?? payload.occupancySync?.status ?? null;
	      const channexFailureMessage = payload.channexProvisioning?.message ?? payload.occupancySync?.message;
	      const channexMessage =
	        channexStatus === "queued"
	          ? " Channel sync queued."
	          : channexStatus === "synced"
	            ? " Channel sync synced."
	            : channexStatus === "failed"
	              ? ` Channel sync failed.${channexFailureMessage ? ` ${channexFailureMessage}` : ""}`
	              : channexStatus === "not_mapped"
	                ? " Channel sync pending mapping."
	                : "";
	      setRoomsMessage(`Room saved.${photoStatus}${channexMessage}`);
	    } catch (error) {
    ...
    navigator.geolocation.getCurrentPosition(
	  ...
	  const uploadPhotos = async (roomId: string, files: FileList | null, kind: "room" | "locality"): Promise<void> => {
	    ...
	    if (roomId.startsWith("temp-")) {
	      stagePhotoUploads(roomId, selectedFiles, kind);
	      return;
	    }

	    const targetRoom = roomDrafts.find((room) => room.id === roomId);
	    if (!targetRoom) {
	      setRoomsMessage("Room not found for photo upload.");
	      return;
	    }
	    ...
	    setRoomsMessage(null);
	    try {
	      const payload = await uploadRoomPhotoFiles(targetRoom, selectedFiles, kind, { unitId: roomId });
  ...
  const promoteRoomPhoto = async (roomId: string, photo: string): Promise<void> => {
    ...
    if (roomId.startsWith("temp-")) {
	  ...
	  const retryPendingPhotoUpload = async (room: RoomFormState): Promise<void> => {
	    if (room.id.startsWith("temp-")) {
	      setRoomsMessage("Save the room before retrying selected photos.");
	      return;
	    }
	    setRoomsSaving(true);
	    setRoomsMessage("Photos uploading...");
	    try {
	      const result = await uploadPendingPhotosForRoom(room, { unitId: room.id, clientId: room.id });
	      if (result.stayUnit) {
	        const uploadedRoom = roomToForm(result.stayUnit);
	        setRoomDrafts((current) => current.map((item) => (item.id === room.id ? uploadedRoom : item)));
	      }
	      setRoomsMessage(result.uploaded ? "Photos saved." : "No pending photos to upload.");
	    } catch (error) {
	      setRoomsMessage(error instanceof Error ? `Photo upload failed: ${error.message}` : "Photo upload failed. Retry from Room photos.");
	    } finally {
	      setRoomsSaving(false);
	    }
	  };
      ...
      <section className={dashboardStyles.glassCard} style={{ padding: "36px", borderRadius: "28px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "16px", flexWrap: "wrap" }}>
          ...
          <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
            ...
            >
              <Plus size={16} />
              {proCreateStatus != null ? "Add Pro room" : "Add room"}
            </button>
          </div>
        </div>
      ...
      <section id="dashboard-rooms" className={dashboardStyles.glassCard} style={{ padding: compactMode ? "20px" : "32px" }}>
        ...
        <div style={{ display: "grid", gap: "20px", minWidth: 0 }}>
	          {visibleRoomDrafts.map((room, index) => {
	            ...
	            const pendingUploads = pendingPhotoUploads[room.id] ?? [];
                      ...
                      >
	                    ...
	                    {pendingUploads.length > 0 ? (
	                      <div style={{ display: "grid", gap: "12px", minWidth: 0 }}>
	                        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "10px", flexWrap: "wrap", minWidth: 0 }}>
	                          <span style={{ fontSize: 12, color: mutedTextColor, fontWeight: 800 }}>
	                            {pendingUploads.length} selected photo{pendingUploads.length === 1 ? "" : "s"} will upload after save.
	                          </span>
	                          {!room.id.startsWith("temp-") ? (
	                            <button type="button" onClick={() => void retryPendingPhotoUpload(room)} disabled={roomsSaving} style={{ ...photoActionButtonStyle, background: "#165dcc", color: "#fff" }}>
	                              Retry upload
	                            </button>
	                          ) : null}
	                        </div>
	                        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(120px, 120px))", gap: "12px", minWidth: 0 }}>
	                          {pendingUploads.map((upload) => (
	                            <div key={upload.id} style={{ display: "grid", gap: "8px", padding: "8px", borderRadius: "16px", border: isProDark ? "1px dashed rgba(147, 197, 253, 0.3)" : "1px dashed #93c5fd", background: isProDark ? "rgba(15, 23, 42, 0.56)" : "#eff6ff", minWidth: 0 }}>
	                              {/* eslint-disable-next-line @next/next/no-img-element */}
	                              <img src={upload.previewUrl} alt={upload.kind === "room" ? "Selected room photo" : "Selected locality photo"} style={{ width: "100%", aspectRatio: "4 / 3", objectFit: "cover", borderRadius: "12px", display: "block" }} />
	                              <button type="button" onClick={() => removePendingUpload(room.id, upload.id)} disabled={roomsSaving} style={{ ...photoActionButtonStyle, background: "rgba(255,255,255,0.92)", color: "#991b1b" }}>
	                                Remove
	                              </button>
	                            </div>
	                          ))}
                  ...
                  <div style={roomSectionPanelStyle}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0, 1fr))", gap: "14px", minWidth: 0 }}>
                      ...
                      <label style={{ display: "flex", flexDirection: "column", gap: "6px", minWidth: 0 }}>
                        ...
                        <input className={dashboardStyles.inputField} style={roomFieldStyle} inputMode="decimal" value={room.priceMorning} onChange={(event) => updateRoomField(room.id, "priceMorning", event.target.value)} />
                      </label>
                      <label style={{ display: "flex", flexDirection: "column", gap: "6px", minWidth: 0 }}>
                        <span style={roomFieldLabelStyle}>Mid-demand reference</span>
                        <input className={dashboardStyles.inputField} style={roomFieldStyle} inputMode="decimal" value={room.priceAfternoon} onChange={(event) => updateRoomField(room.id, "priceAfternoon", event.target.value)} />
                      </label>
                      <label style={{ display: "flex", flexDirection: "column", gap: "6px", minWidth: 0 }}>
                        <span style={roomFieldLabelStyle}>Higher-demand reference</span>
                        <input className={dashboardStyles.inputField} style={roomFieldStyle} inputMode="decimal" value={room.priceEvening} onChange={(event) => updateRoomField(room.id, "priceEvening", event.target.value)} />
                      </label>
                    </div>
                  ...
                  <div style={footerBarStyle}>
                    <div style={{ display: "flex", gap: "8px", alignItems: "center", flexWrap: "wrap" }}>
                      <button type="button" className={dashboardStyles.primaryBtn} style={{ width: "auto", minWidth: "auto", minHeight: "44px", padding: "0 18px", borderRadius: "14px", fontSize: "13px", fontWeight: 800 }} onClick={() => void saveRoom(room)} disabled={roomsSaving}>
                        {roomsSaving
                          ? "Saving..."
                          : room.id.startsWith("temp-") && proCreateStatus === "active"
                            ? "Pay to Add Room"
                            : "Save"}
                      </button>
                      {roomsMessage ? (
