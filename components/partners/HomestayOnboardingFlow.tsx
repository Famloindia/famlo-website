...
import { buildPlatformAgreementCompatibilityPatch, isValidGstin, normalizeGstin } from "@/lib/host-onboarding-legal";
import styles from "./onboarding.module.css";

const stepLoadingFallback = (
  <div className={styles.stepLoadingState}>
    <Loader2 className="animate-spin" size={18} />
    <span>Loading onboarding step...</span>
  </div>
);

const Step1Hook = dynamic(() => import("./homes/steps/Step1Hook"), {
  loading: () => stepLoadingFallback,
});
const Step2Story = dynamic(() => import("./homes/steps/Step2Story"), {
  loading: () => stepLoadingFallback,
});
const Step3Space = dynamic(() => import("./homes/steps/Step3Space"), {
  loading: () => stepLoadingFallback,
});
...
export default function HomestayOnboardingFlow(): React.JSX.Element {
  ...
  const buildCompliancePatch = () => ({
    ...
    gstin: normalizeGstin(resolvedFlow.gstin),
    ...buildPlatformAgreementCompatibilityPatch(
      resolvedFlow.platformAgreementAccepted,
      resolvedFlow.platformAgreementAcceptedAt || resolvedFlow.hostAgreementAcceptedAt
    ),
  });
  ...
  const handleNext = async () => {
    ...
    if (step === 3) {
      ...
      if (!isValidGstin(resolvedFlow.gstin)) {
        setError("Enter a valid GSTIN or leave the field empty if it does not apply.");
        return;
      }
      if (!resolvedFlow.platformAgreementAccepted) {
        setError("Please accept the Famlo platform agreement to finish.");
        return;
      }

      const agreementAcceptedAt =
        resolvedFlow.platformAgreementAcceptedAt ||
        resolvedFlow.hostAgreementAcceptedAt ||
        new Date().toISOString();

      setFlow((current) => ({
        ...current,
        gstin: normalizeGstin(current.gstin),
        ...buildPlatformAgreementCompatibilityPatch(true, agreementAcceptedAt),
      }));
