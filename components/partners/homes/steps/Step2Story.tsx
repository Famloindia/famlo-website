...
import {
  HOST_REEL_ACCEPT_ATTRIBUTE,
  MAX_HOST_REEL_UPLOAD_BYTES,
...
async function uploadFiles(files: File[], folder: string): Promise<string[]> {
  ...
  for (const file of files) {
    ...
    if (file.size > MAX_IMAGE_UPLOAD_BYTES) {
      throw new Error(`Images must be ${formatImageUploadLimitLabel()} or smaller.`);
    }
    ...
    const uploadFile =
      lowerName.endsWith(".heic") || lowerName.endsWith(".heif")
        ...
        : await compressImageForUpload(file, {
            maxWidth: folder === "room-photos" ? 1600 : 1400,
            quality: 0.82,
            mimeType: "image/webp",
          });

    const sendUpload = async (nextFile: File) => {
    ...
    try {
      results.push(await sendUpload(uploadFile));
    } catch (error) {
      ...
      results.push(await sendUpload(file));
    }
  }
...
async function uploadHostReelDirect(params: {
  draftId: string;
  file: File;
}): Promise<{
  familyId: string;
  ...
  const uploadTargetResponse = await fetch("/api/onboarding/home/reel-upload-url", {
    method: "POST",
  ...
  const uploadTargetPayload = (await uploadTargetResponse.json().catch(() => ({}))) as {
    error?: string;
    uploadUrl?: string;
    storageKey?: string;
    publicUrl?: string;
  ...
  if (
    ...
    !uploadTargetPayload.uploadUrl ||
    !uploadTargetPayload.storageKey ||
    !uploadTargetPayload.publicUrl ||
    !uploadTargetPayload.familyId
  ) {
    throw new Error(uploadTargetPayload.error || "Unable to prepare the host reel upload.");
  }

  const uploadUrl = uploadTargetPayload.uploadUrl;
  const storageKey = uploadTargetPayload.storageKey;
  const publicUrl = uploadTargetPayload.publicUrl;
  const familyId = uploadTargetPayload.familyId;
  ...
  const uploadWithFallbackRoute = async (): Promise<{
    familyId: string;
    hostReelStorageKey: string;
    hostReelPublicUrl: string;
    hostReelMimeType: string;
    ...
    const response = await fetch("/api/onboarding/home/reel-upload-fallback", {
      method: "POST",
      body: formData,
    });
    ...
    if (
      !response.ok ||
      !payload.hostReelStorageKey ||
      !payload.hostReelPublicUrl ||
      !payload.hostReelMimeType ||
      typeof payload.hostReelSizeBytes !== "number" ||
      !payload.hostReelUploadedAt
    ) {
      throw new Error(payload.error || "Unable to upload the host reel.");
    }

    return {
      familyId,
      hostReelStorageKey: payload.hostReelStorageKey,
      hostReelPublicUrl: payload.hostReelPublicUrl,
      hostReelMimeType: payload.hostReelMimeType,
      hostReelSizeBytes: payload.hostReelSizeBytes,
      hostReelUploadedAt: payload.hostReelUploadedAt,
      width: null,
      height: null,
  ...
  const readVideoMetadata = (file: File) =>
    new Promise<{ width: number | null; height: number | null; durationSeconds: number | null }>((resolve) => {
  ...
  const metadata = await readVideoMetadata(params.file);

  const uploadWithXhr = (url: string, file: File) =>
  ...
  try {
    ...
    if (!uploadResponse.ok) {
      throw new Error("Host reel upload failed. Please retry with a smaller MP4, WEBM, or MOV file.");
    }
    console.info("[onboarding-host-reel] r2-upload:success", {
      draftId: params.draftId,
      familyId,
      storageKey,
      publicUrl,
    ...
    const shouldRetryWithXhr = error instanceof Error && /failed to fetch/i.test(error.message);
    if (!shouldRetryWithXhr) {
      throw error;
    }
    try {
      await uploadWithXhr(uploadUrl, params.file);
      console.info("[onboarding-host-reel] r2-upload:success", {
        draftId: params.draftId,
        familyId,
        storageKey,
      ...
      return uploadWithFallbackRoute();
    }
  }

  return {
    familyId,
    hostReelStorageKey: storageKey,
    hostReelPublicUrl: publicUrl,
    hostReelMimeType: params.file.type,
    hostReelSizeBytes: params.file.size,
...
export default function Step2Story({ data, update }: any) {
  ...
  const hostReelInputRef = useRef<HTMLInputElement | null>(null);
  ...
  const hostReelPublicUrl = String(data.hostReelPublicUrl ?? "");
  const hostReelMimeType = String(data.hostReelMimeType ?? "");
  const hostReelSizeBytes = Number(data.hostReelSizeBytes ?? 0);
  ...
  const handleHostReelUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    if (!file) return;

    if (!data.draftId && !data.id && !data.hostDraftId) {
      setUploadError("Save Step 1 first so we can attach the reel to your host draft.");
      event.target.value = "";
      return;
    }

    setUploadError(null);
    setUploading("host-reel");

    try {
      const nextReel = await uploadHostReelDirect({
        draftId: String(data.draftId ?? data.id ?? data.hostDraftId ?? ""),
        file,
      });
      console.info("[onboarding-host-reel] property-reels:request", {
        draftId: String(data.draftId ?? data.id ?? data.hostDraftId ?? ""),
        familyId: nextReel.familyId,
        storageKey: nextReel.hostReelStorageKey,
        publicUrl: nextReel.hostReelPublicUrl,
      });
      const persistResponse = await fetch("/api/host/property-reels", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          draftId: String(data.draftId ?? data.id ?? data.hostDraftId ?? ""),
          familyId: nextReel.familyId,
          storage_key: nextReel.hostReelStorageKey,
          public_url: nextReel.hostReelPublicUrl,
          mime_type: nextReel.hostReelMimeType,
          size_bytes: nextReel.hostReelSizeBytes,
          width: nextReel.width,
          height: nextReel.height,
          duration_seconds: nextReel.durationSeconds,
        }),
      });
      const persistPayload = (await persistResponse.json().catch(() => ({}))) as { error?: string; branch?: string };
      if (!persistResponse.ok) {
        console.error("[onboarding-host-reel] property-reels:failure", {
          status: persistResponse.status,
          body: persistPayload,
        });
        throw new Error(persistPayload.error || "Host reel metadata save failed.");
      }
      update("hostReelStorageKey", nextReel.hostReelStorageKey);
      update("hostReelPublicUrl", nextReel.hostReelPublicUrl);
      update("hostReelMimeType", nextReel.hostReelMimeType);
      update("hostReelSizeBytes", nextReel.hostReelSizeBytes);
      update("hostReelUploadedAt", nextReel.hostReelUploadedAt);
    } catch (error) {
      setUploadError(error instanceof Error ? error.message : "Host reel upload failed.");
    } finally {
      setUploading(null);
      event.target.value = "";
    }
    ...
    <div className={styles.animateIn}>
      ...
      <div className={styles.formGrid}>
        ...
        <section className={`${styles.formGroup} ${styles.fullWidth} ${styles.panelSection}`}>
          ...
          <div className={styles.photoUploader}>
            <div className={styles.chipGroupTitle}>Host Reel</div>
            <div className={styles.photoUploadBar}>
              <button
                type="button"
                className={styles.primaryMiniBtn}
                onClick={() => hostReelInputRef.current?.click()}
              >
                <Upload size={14} />
                {hostReelPublicUrl ? "Replace reel" : "Upload reel"}
              </button>
              <input
                ref={hostReelInputRef}
                type="file"
                accept={HOST_REEL_ACCEPT_ATTRIBUTE}
                hidden
                onChange={(event) => void handleHostReelUpload(event)}
              />
              <div className={styles.helperText}>
                Upload a short vertical video of the home or experience. MP4, WEBM, and MOV are supported up to {Math.round(MAX_HOST_REEL_UPLOAD_BYTES / (1024 * 1024))}MB.
              </div>
            </div>

            {uploading === "host-reel" ? <div className={styles.uploadState}>Uploading host reel directly to secure storage...</div> : null}
            {hostReelPublicUrl ? (
              <div className={styles.reelCard}>
                <div className={styles.reelPreviewWrap}>
                  <video
                    src={hostReelPublicUrl}
                ...
                <div className={styles.reelMeta}>
                  <div>
                    <div className={styles.reelLabel}>Saved</div>
                    <div className={styles.reelValue}>
                      {hostReelMimeType ? hostReelMimeType.replace("video/", "").toUpperCase() : "Video"} reel ready for review
                    </div>
                  </div>
                  <div className={styles.reelPill}>
                    {hostReelSizeBytes > 0 ? `${(hostReelSizeBytes / (1024 * 1024)).toFixed(1)}MB` : "Uploaded"}
                  </div>
                </div>
              </div>
            ) : null}
