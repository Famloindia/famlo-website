...
import {
  canDirectUploadHostProfileImage,
  validateHostProfileDirectUpload,
} from "@/lib/host-profile-upload-shared";
...
export default function Step1Profile({ data, update, onStepComplete }: any) {
  ...
  const uploadHostPhoto = async (file: File) => {
    const fallbackUpload = () => uploadFile(file, "host-profiles", "Failed to upload profile photo.");

    if (!canDirectUploadHostProfileImage({ fileName: file.name, mimeType: file.type })) {
    ...
    try {
      ...
      const targetResponse = await fetch("/api/onboarding/home/profile-photo-upload-url", {
        method: "POST",
      ...
      if (!targetResponse.ok || !targetPayload.uploadUrl || !targetPayload.publicUrl) {
        throw new Error(targetPayload.error || "Unable to prepare profile photo upload.");
      }

      const uploadResponse = await fetch(targetPayload.uploadUrl, {
        method: "PUT",
        headers: {
      ...
      return targetPayload.publicUrl;
    } catch (directUploadError) {
      console.warn("[host-photo-upload] Falling back to relay upload.", directUploadError);
      return fallbackUpload();
    }
  };
  ...
  const captureLivePhoto = async () => {
    ...
    try {
      ...
      if (cameraTarget === "idDocument") {
        ...
        const url =
          cameraTarget === "hostPhoto"
            ? await uploadHostPhoto(file)
            : await uploadFile(file, "live-verifications", "Failed to upload the captured photo.");
